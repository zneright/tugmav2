...
interface Applicant {
  ...
  email: string;
  course: string;
  profile_photo?: string; // 👈 Added profile photo support
  resume_name: string;
  resume_data: string;
  job_title: string;
...
export default function EmployerApplicants() {
  ...
  const [downloadingId, setDownloadingId] = useState<number | null>(null); // 👈 Track downloads
    ...
    try {
      ...
      if (res.ok) {
        ...
        const safeData = data.map((app: any) => {
          ...
          return {
            ...app,
            profile_photo: app.profilePhoto || app.profile_photo || null, // 👈 Maps photo from backend
  ...
  // 👇 🔥 SECURE RESUME DOWNLOAD BYPASS (No Iframe crashes) 🔥 👇
            ...
            <div className="relative">
              ...
              <div className="px-6 sm:px-10 pb-6 -mt-12 relative z-10 flex flex-col sm:flex-row gap-5 sm:items-end">
                <div className={`w-24 h-24 rounded-2xl flex items-center justify-center font-black text-4xl border-4 border-white dark:border-zinc-900 shadow-lg shrink-0 overflow-hidden ${!selectedApplicant.profile_photo ? ['bg-blue-100 text-blue-600', 'bg-emerald-100 text-emerald-600', 'bg-purple-100 text-purple-600', 'bg-amber-100 text-amber-600'][getAvatarColor(selectedApplicant.application_id)] : 'bg-zinc-200'}`}>
                  {/* 🔥 FIX: Load profile photo if it exists 🔥 */}
                  {selectedApplicant.profile_photo ? (
                    <img src={selectedApplicant.profile_photo} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  {selectedApplicant.resume_data && (
                    <button
                      ...
                      className="px-4 py-2 bg-zinc-900 hover:bg-black text-white text-xs font-bold rounded-xl transition-colors shrink-0 flex items-center gap-1 disabled:opacity-50"
                    >
              ...
              return (
                <div key={app.application_id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 md:p-5 items-center hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group">
                  ...
                  <div className="col-span-1 md:col-span-3 flex items-center gap-4">
                    {/* 🔥 FIX: Render Profile Photo in List View 🔥 */}
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shrink-0 overflow-hidden ${!app.profile_photo ? ['bg-blue-100 text-blue-600', 'bg-emerald-100 text-emerald-600', 'bg-purple-100 text-purple-600', 'bg-amber-100 text-amber-600'][getAvatarColor(app.application_id)] : 'bg-zinc-200'}`}>
                      {app.profile_photo ? (
                        <img src={app.profile_photo} alt={app.first_name} className="w-full h-full object-cover" />
                      ) : (
