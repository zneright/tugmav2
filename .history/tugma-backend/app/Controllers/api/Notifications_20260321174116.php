...
class Notifications extends ResourceController
{
    ...
    public function getBroadcastRecipients()
    {
        ...
        if (!$title || !$message || !$sender_uid) return $this->fail('Missing parameters');

        $db = \Config\Database::connect();
        try {
            $recipients = $db->table('notifications n')
                ->select('u.first_name, u.last_name, u.email, n.is_read, n.created_at')
                ->join('users u', 'u.firebase_uid = n.user_uid')
                ->where([
