...
class Admin extends ResourceController
{
    ...
    public function getAllUsers()
    {
        ...
        try {
            ...
            foreach ($users as $u) {
                ...
                // Intelligently format the name based on their role
                if ($role === 'Employer') {
                ...
                // Safely format the date if the column exists
                $joined = isset($u['created_at']) ? date('M d, Y', strtotime($u['created_at'])) : 'Recently';
                
                // Check if a status column exists, otherwise default to Active
    ...
    public function updateUserStatus($uid = null)
    {
        ...
        try {
            // HACKATHON MAGIC: Automatically create the 'status' column if it's missing!
    ...
    public function deleteUser($uid = null)
    {
        ...
        $db = \Config\Database::connect();
        try {
            // Delete the user and their associated profiles to keep the database clean
    ...
    // 🔥 Fetch all Support Tickets submitted by users
        ...
        try {
            ...
            foreach ($tickets as $t) {
                ...
                $formattedTickets[] = [
                    ...
                    // 👇 THIS IS THE MISSING LINE THAT FIXES THE 400 CRASH 👇
    ...
    public function resolveTicket($ticket_id = null)
    {
        ...
        $db = \Config\Database::connect();
        try {
            // Using is_read = 1 to represent a "Closed/Resolved" ticket
    ...
    // 🔥 Mark a Ticket as "Replied" and send message to user
    public function replyTicket($ticket_id = null)
    {
        ...
        try {
            // 1. Send the reply notification straight to the Employer/Student's inbox
            $db->table('notifications')->insert([
                'user_uid'   => $receiverUid,
                'sender_uid' => 'ADMIN_SYSTEM', 
                'title'      => 'Support Response: ' . $subject,
                'message'    => $replyMessage,
                'type'       => 'alert', // Renders as an official Amber alert in their inbox!
                'is_read'    => 0
            ]);

            // 2. Mark the original ticket as resolved (Closed)
            $db->table('notifications')->where('id', $ticket_id)->update(['is_read' => 1]);
    ...
    public function getAuditLogs()
    {
        $db = \Config\Database::connect();
        try {
            // HACKATHON MAGIC: Auto-create the table if it doesn't exist!
            ...
            // Fetch the latest 100 logs and attach the user's name/role
    ...
    // 🔥 NEW: Endpoint to record a new log (Call this from anywhere in React!)
