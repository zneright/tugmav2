...
class Admin extends ResourceController
{
    ...
    public function getSupportTickets()
    {
        ...
        try {
            ...
            $tickets = $db->table('notifications n')
                ...
                ->where('n.type', 'ticket')
                ->orderBy('n.is_read', 'ASC') // Unread (Open) tickets at the top
                ->orderBy('n.created_at', 'DESC')
                ->get()->getResultArray();

            $formattedTickets = [];
            foreach ($tickets as $t) {
                // Format the user's name based on their role
                $name = 'Unknown User';
                if ($t['role'] === 'employer') {
    ...
    public function replyTicket($ticket_id = null)
    {
        ...
        $db = \Config\Database::connect();
        try {
            // Optional: If you ever add a 'status' text column to notifications, 
            // you could update it here to 'In Progress'. 
            // For now, returning 200 OK successfully stops the React 404 crash!
            return $this->respond(['message' => 'Reply action recorded successfully']);
