...
class Users extends ResourceController
{
    ...
    // ARCHIVE EVERYTHING AND DELETE USER
    public function deleteProfile($uid = null)
    {
        ...
        try {
            ...
            // 1. Fetch existing core user data
            ...
            // 2. Fetch ALL associated data across the database to archive
            ...
            // --- NEW: Fetch ATS History (assuming table name is ats_history and is linked by firebase_uid) ---
            $atsHistory = [];
            if ($db->tableExists('ats_history')) {
                // If you don't save firebase_uid in ats_history, you'll need to adjust this query.
                // Assuming it's linked by user ID or similar. If not, this array will just be empty.
                // If ats_history is NOT currently linked to a specific user, you'll need to modify your ATS model first to save the UID.
                 try {
                     $atsHistory = $db->table('ats_history')->where('firebase_uid', $uid)->get()->getResultArray();
                 } catch (\Exception $e) {
                     // Table might not have firebase_uid, fail gracefully for this specific fetch
                     $atsHistory = ["error" => "Could not link ATS history to user."];
                 }
            }
            
            // Global Data (Messages & Notifications where they are either sender or receiver)
            ...
            // 3. Compile everything into one massive JSON payload
            $comprehensiveArchive = [
                ...
                'dtr_logs'         => $dtrLogs,
                'ats_history'      => $atsHistory, // <-- Added ATS History to the archive payload
                'messages'         => $messages,
                'notifications'    => $notifications
            ];

            // 4. Prepare data for the archive table
            $archivedData = [
                ...
                'profile_data'  => json_encode($comprehensiveArchive), // Massive JSON string
            ...
            // 6. Systematically DELETE from all active tables to prevent orphans
