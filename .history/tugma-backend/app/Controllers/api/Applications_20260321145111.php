...
class Applications extends ResourceController
{
    ...
    // 👇 NEW HELPER: Downloads and extracts text from Cloudinary PDFs!
    private function extractPdfText($pdfUrl) {
        try {
            // Fetch the PDF file from the secure URL
            $pdfContent = file_get_contents($pdfUrl);
            if (!$pdfContent) return "Could not download resume.";
            
            // Save it to a temporary file for the Parser to read
            ...
            // Limit to 3000 chars so we don't overload Gemini's token limit
            return substr(trim($text), 0, 3000);
        } catch (\Exception $e) {
            return "Resume extraction failed or PDF is locked/unreadable.";
        }
    }
