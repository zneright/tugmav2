...
class Applications extends ResourceController
{
   ...
   // 🔥 FIXED: Fetch all applications for a specific STUDENT 🔥
    public function getStudentApplications($studentUid = null)
    {
        $db = \Config\Database::connect();

        try {
            // 👇 CHANGED FROM 'applications' TO 'job_interactions' 👇
            $builder = $db->table('job_interactions'); 
            
            $builder->select('
                job_interactions.id as application_id,
                job_interactions.status,
                job_interactions.created_at as applied_at,  /* Note: Use created_at if applied_at doesnt exist */
                employer_jobs.title as job_title,
                employer_jobs.location,
                employer_jobs.firebase_uid as employer_uid,
                users.first_name as company_name_fallback,
                employer_profiles.company_name,
                employer_profiles.industry
            ');
            
            // 👇 UPDATED JOINS TO MATCH 'job_interactions' AND 'employer_jobs' 👇
            $builder->join('employer_jobs', 'employer_jobs.id = job_interactions.job_id', 'left');
            ...
            // IF YOUR TABLE TRACKS VIEWS AND APPLIES, UNCOMMENT THIS LINE:
            // $builder->where('job_interactions.type', 'applied'); 
            ...
            foreach ($data as $row) {
                $compName = !empty($row['company_name']) ? $row['company_name'] : (!empty($row['company_name_fallback']) ? $row['company_name_fallback'] : 'Unknown Company');
                ...
                $formatted[] = [
                    ...
                    'rating' => '4.' . rand(0, 9) // Mock rating for visual flair
                ];
            }
            ...
            return $this->failServerError('Error fetching student applications: ' . $e->getMessage());
        }
    }
    // 🔥 BULLETPROOF: Fetch all applications for a specific STUDENT 🔥
    public function getStudentApplications($studentUid = null)
    {
        $db = \Config\Database::connect();
        ...
        try {
            ...
            // SAFEST QUERY: We use job_interactions.* to avoid crashing if a specific date/status column is missing
            $builder->select('
                job_interactions.*,
                employer_jobs.title as job_title,
                employer_jobs.location,
            ...
            // Join only the guaranteed tables
            $builder->join('employer_jobs', 'employer_jobs.id = job_interactions.job_id', 'left');
            // We pull the company name directly from your users table (from your earlier setup)
            $builder->join('users', 'users.firebase_uid = employer_jobs.firebase_uid', 'left');
            ...
            foreach ($data as $row) {
                ...
                $compName = !empty($row['company_name']) ? $row['company_name'] : 'Unknown Company';
                
                $formatted[] = [
                    'id' => $row['id'] ?? rand(1000, 9999),
                    'employer_uid' => $row['employer_uid'] ?? '',
                    'company_name' => $compName,
                    'industry' => 'Technology', // Safe fallback
                    'location' => $row['location'] ?? 'Remote',
                    'status' => $row['status'] ?? 'Applied', // Defaults to 'Applied' if status column is missing
            ...
            // 🔥 CATCH \Throwable: This forces PHP to output the EXACT MySQL crash reason!
