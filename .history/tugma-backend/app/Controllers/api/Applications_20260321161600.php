...
class Applications extends ResourceController
{
    ...
    public function getEmployerApplicants($employer_uid = null)
    {
        ...
        try {
            ...
            $builder->select('ji.id as application_id, ji.created_at as applied_at, ji.status, ji.ai_match_score, ji.ai_assessment,
                              ...
                              sp.resume_name, 
                              sp.profile_data as resume_data, 
    ...
    public function updateStatus($application_id = null)
    {
        ...
        // 1. Update the application status to Hired/Rejected/etc
        $db->table('job_interactions')->where('id', $application_id)->update(['status' => $json['status']]);

        // 🔥 2. AUTO-ACTIVATE OJT TRACKER IF HIRED 🔥
        if ($json['status'] === 'Hired') {
            // Find out WHICH student got hired
            $interaction = $db->table('job_interactions')
                              ->select('student_uid')
                              ->where('id', $application_id)
                              ->get()
                              ->getRowArray();
            
            if ($interaction && !empty($interaction['student_uid'])) {
                $studentUid = $interaction['student_uid'];
                
                // Grab their current student profile
                $profile = $db->table('student_profiles')
                              ->select('ojt_data')
                              ->where('firebase_uid', $studentUid)
                              ->get()
                              ->getRowArray();
                
                if ($profile) {
                    // Decode their OJT data, change status, and save it back!
                    $ojtData = !empty($profile['ojt_data']) ? json_decode($profile['ojt_data'], true) : [];
                    if (!is_array($ojtData)) $ojtData = [];
                    
                    $ojtData['status'] = 'In Progress'; // 👈 This triggers the React dashboard!
                    
                    $db->table('student_profiles')
                       ->where('firebase_uid', $studentUid)
                       ->update(['ojt_data' => json_encode($ojtData)]);
                }
            }
        }
    ...
    public function analyzeApplicant($applicationId = null)
    {
        ...
        try {
            ...
            $builder->select('
                ...
                student_profiles.profile_data as resume_data,
                users.first_name,
                users.last_name
            ');
            ...
            if (!empty($resumeUrl)) {
                try {
                    ...
                    if ($fileContent) {
                        ...
                        $isPdf = stripos($resumeUrl, '.pdf') !== false || stripos($resumeUrl, 'application/pdf') !== false;
            ...
            RESUME TEXT:
            " . ($hasResume ? $resumeText : "[NO RESUME DATA AVAILABLE]") . "
            ...
            " . ($hasResume ? 
                "3. A resume file WAS successfully parsed. You MUST evaluate BOTH the Candidate Profile data and the extracted Resume Text to calculate the score and write your assessment." : 
                "3. CRITICAL: No readable resume was attached. You MUST base your score entirely on the Candidate Profile. You MUST explicitly state in your overall_assessment: 'Note: No resume was attached, so this analysis is based solely on the applicant's profile data.'") . "
  ...
  public function getDashboardStats($studentUid = null)
{
    ...
    // 1. Get Application Status Counts
    ...
    // 2. Get Real Total Credited Hours from DTR Logs
    $hoursRaw = $db->table('dtr_logs')
        ->selectSum('hoursCredited')
        ->where('student_uid', $studentUid)
        ->get()
        ->getRow();
    
    $totalCompletedHours = (int)($hoursRaw->hoursCredited ?? 0);

    // 3. Get OJT Required Hours from Profile
    $profile = $db->table('student_profiles')
                  ->select('ojt_data')
                  ->where('firebase_uid', $studentUid)
                  ->get()
    ...
    $requiredHours = 450; // Default
    if ($profile && !empty($profile['ojt_data'])) {
        $ojt = json_decode($profile['ojt_data'], true);
        $requiredHours = (int)($ojt['requiredHours'] ?? 450);
    }
    ...
    $stats = [
        ...
        'pending' => 0, 
        'reviewed' => 0,
        'total_hours' => $totalCompletedHours,
