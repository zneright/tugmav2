...
class Users extends ResourceController
{
    ...
    public function deleteProfile($uid = null)
    {
        ...
        try {
            ...
            // Fetch student profile data to archive as well
            $studentProfile = $db->table('student_profiles')->where('firebase_uid', $uid)->get()->getRowArray();

            // 2. Prepare data for the archive table
            $archivedData = [
                ...
                // Store their specific student data as a JSON string for record-keeping
                'profile_data'  => $studentProfile ? json_encode($studentProfile) : json_encode([]),
                'archived_at'   => date('Y-m-d H:i:s'),
                'reason'        => 'User initiated self-deletion'
            ];

            // 3. Start Database Transaction (ensures all or nothing happens)
            $db->transStart();
            ...
            return $this->respondDeleted([
                'status' => 200,
                'message' => 'User data successfully archived and removed from active records.'
            ]);
