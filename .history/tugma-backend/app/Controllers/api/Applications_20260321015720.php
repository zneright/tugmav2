...
// 🔥 BULLETPROOF & FIXED: Fetch all applications for a specific STUDENT 🔥
    ...
    public function getDashboardStats($studentUid = null)
    {
        ...
        // 1. Get Status Counts (Shortlisted, Hired, etc.)
        $statusCounts = $db->table('job_interactions')
        ...
        // 2. Get Interaction Counts (Views vs Applies)
        $interactionCounts = $db->table('job_interactions')
        ...
        foreach ($interactionCounts as $row) {
            if ($row['interaction_type'] === 'applied') $stats['applied'] = (int)$row['total'];
            if ($row['interaction_type'] === 'viewed') $stats['viewed'] = (int)$row['total'];
        }
        ...
        foreach ($statusCounts as $row) {
            ...
            if ($row['status'] === 'New Applicant' || $row['status'] === 'Reviewed') $stats['pending'] += (int)$row['total'];
        }
