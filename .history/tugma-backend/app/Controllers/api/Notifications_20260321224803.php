...
class Notifications extends ResourceController
{
    ...
    public function getBroadcastRecipients()
    {
        ...
        // 🔥 FIX: Removed $message requirement
        ...
        try {
            $recipients = $db->table('notifications n')
                ->select('
                    ...
                    n.created_at
                ')
                ->join('users u', 'u.firebase_uid = n.user_uid', 'left') // Changed to LEFT join for safety
                ->where([
    ...
    public function sendGlobalBroadcast()
    {
        ...
        try {
            ...
            // Filter by role if not sending to ALL
            ...
            foreach ($users as $user) {
                $insertData[] = [
                    ...
                    'title' => $data['title'],
                    'message' => $data['message'],
                    'type' => 'alert', // 🔥 'alert' type renders as red/urgent for users!
                    'is_read' => 0
                ];
            }
    ...
    public function submitTicket()
    {
        ...
        try {
            ...
            // Fallback in case no admin exists yet in the DB
            if (empty($admins)) {
            ...
            foreach ($admins as $admin) {
                $insertData[] = [
                    ...
                    'type'       => 'ticket', // Use a special type for tickets
                    'is_read'    => 0
                ];
            }
    ...
    public function getUserNotifications($uid = null)
    {
        ...
        try {
            $notifications = $db->table('notifications n')
                ->select('n.*, u.first_name, u.last_name, u.company_name, u.email as sender_email, u.role as sender_role')
                ->join('users u', 'u.firebase_uid = n.sender_uid', 'left')
                ->where('n.user_uid', $uid)
                ->orderBy('n.created_at', 'DESC')
                ->get()->getResultArray();

            // Format the sender's name cleanly based on their role
            foreach ($notifications as &$n) {
                if (!empty($n['sender_role'])) {
                    if ($n['sender_role'] === 'employer') {
                        $n['sender_name'] = !empty($n['company_name']) ? $n['company_name'] : $n['sender_email'];
                    } else {
                        $n['sender_name'] = trim(($n['first_name'] ?? '') . ' ' . ($n['last_name'] ?? ''));
                        if (empty($n['sender_name'])) $n['sender_name'] = $n['sender_email'];
                    }
                } else {
