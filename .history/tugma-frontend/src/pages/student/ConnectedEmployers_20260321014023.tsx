...
export default function ConnectedEmployers() {
  ...
  const fetchConnections = async (studentUid: string) => {
    try {
      ...
      // 1. Wait for the response JSON first
      const data = await res.json();

      // 2. If it is a 500 Error, THROW it so we can see it!
      if (!res.ok) {
        throw new Error(data.messages?.error || data.message || "Unknown Server Error");
      }

      // 3. Otherwise, set the data
      setConnections(Array.isArray(data) ? data : []);

    } catch (error: any) {
      console.error("Failed to fetch connections", error);
      // 🔥 Pop an alert so you don't have to guess what's wrong!
