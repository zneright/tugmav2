...
export default function Dashboard() {
  ...
  // 🔥 NEW: Store real profile data for the insights card 🔥
  ...
  const fetchDashboardData = async (userUid: string) => {
    try {
      ...
      if (statsRes.ok) {
        ...
        console.log("📊 REAL STATS FROM PHP:", statsData); // 👈 Check your console!
        setStats(statsData);
      }

      if (profileRes.ok) {
        const profileData = await profileRes.json();
        console.log("👤 REAL PROFILE FROM PHP:", profileData); // 👈 Check your console!
        ...
        <div className="lg:col-span-8 bg-gradient-to-br from-zinc-900 to-black rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl border border-zinc-800">
          <div className="relative z-10">
            ...
            <h1 className="text-4xl font-black mb-2 tracking-tight">
              {stats.hired > 0 ? 'Congratulations! 🎉' : 'System Status: Active'}
            </h1>

            {/* 🔥 DYNAMIC TEXT: Changes based on their real stats! 🔥 */}
            <p className="text-zinc-400 font-medium max-w-md text-lg">
              You have explored <span className="text-white font-bold">{stats.viewed}</span> jobs.
              {activePending > 0 ? (
                <span> You currently have <span className="text-white underline decoration-purple-500 underline-offset-4">{activePending} applications</span> awaiting review.</span>
              ) : stats.hired > 0 ? (
                <span> You are officially hired! Focus on your OJT hours.</span>
              ) : (
                <span> Start applying to get your OJT journey started!</span>
              )}
            </p>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm flex-1">
            ...
            <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800">
              ...
              {/* 🔥 DYNAMIC TEXT: Pulls from their resume/profile array! 🔥 */}
          ...
          <div className="bg-purple-600 rounded-3xl p-6 text-white shadow-xl shadow-purple-500/20">
            ...
            <p className="text-xs text-purple-100 leading-relaxed mb-4">Your dashboard is fully synced. Every time an employer reviews your profile or shortlists you, these numbers update instantly.</p>
