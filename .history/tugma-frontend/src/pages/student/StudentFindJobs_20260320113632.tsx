...
import { Search, MapPin, Filter, Clock, Building2, ChevronRight, CheckCircle2, Eye, Calendar, Sparkles, AlertCircle, Loader2, Tag } from 'lucide-react';
import { auth } from '../../firebaseConfig';
import { onAuthStateChanged } from 'firebase/auth';

// --- 🧠 SMART AI SEMANTIC ENGINE (Knowledge Graph) ---
const SMART_DICTIONARY = {
  locations: {
    "metro manila": ["makati", "taguig", "bgc", "pasay", "manila", "quezon city", "pasig", "mandaluyong", "alabang"],
    ...
    "cebu": ["dasmarinas", "santa rosa", "binan", "carmona", "cebu city", "mandaue", "lapu-lapu"]
  },
  categories: {
    "web": ["react", "frontend", "backend", "fullstack", "php", "javascript", "html", "css", "node", "laravel", "codeigniter"],
    "software": ["c#", ".net", "java", "python", "c++", "software", "desktop", "systems"],
    "design": ["figma", "design", "ui", "ux", "wireframe", "prototype", "photoshop", "illustrator", "graphic"],
    "data": ["sql", "database", "data", "analytics", "excel", "python", "machine learning", "ai"],
    "network": ["network", "hardware", "support", "helpdesk", "troubleshooting", "cisco", "sysadmin", "it"]
  }
};
...
interface Job {
  ...
  skills: string[];
  companyName?: string;
  matchScore?: number; // Used for our new Weighted Engine
}
...
export default function FindJobs() {
  ...
  const [categoryQuery, setCategoryQuery] = useState(''); // 👈 NEW: Now a text input!
  ...
  // --- 🧠 THE WEIGHTED RECOMMENDATION ENGINE ---
  const displayJobs = useMemo(() => {
    if (!studentProfile) return [];

    const isSearching = searchQuery.trim() !== '' || locationQuery.trim() !== '' || categoryQuery.trim() !== '';

    return allJobs.map(job => {
      let score = 0;

      // 1. Strict Tab Filtering (If they explicitly want remote, don't score onsite jobs)
      if (activeTab !== 'All') {
        if (activeTab === 'Paid Allowance' && job.salary === 'Unpaid / Volunteer') return { ...job, matchScore: -1 };
        if (['Remote', 'Hybrid', 'Onsite'].includes(activeTab) && job.work_setup !== activeTab) return { ...job, matchScore: -1 };
      }

      // 2. Keyword Search Scoring (+30 points)
      if (searchQuery) {
        const q = searchQuery.toLowerCase();
        if (job.title.toLowerCase().includes(q) || job.companyName?.toLowerCase().includes(q)) score += 30;
        if (job.skills.some(s => s.toLowerCase().includes(q))) score += 20;
      }

      // 3. Category/Title Smart Scoring (+40 points)
      if (categoryQuery) {
        const cat = categoryQuery.toLowerCase();
        const jobText = `${job.title} ${job.skills.join(' ')}`.toLowerCase();

        // Direct Match
        if (jobText.includes(cat)) score += 40;

        // Semantic Match (e.g. typing "web" matches "react" jobs)
        Object.keys(SMART_DICTIONARY.categories).forEach(key => {
          if (cat.includes(key) || key.includes(cat)) {
            // @ts-ignore
            const keywords = SMART_DICTIONARY.categories[key];
            if (keywords.some((kw: string) => jobText.includes(kw))) {
              score += 35; // Semantic matches get slightly less points than direct matches
            }
          }
        });
      }

      // 4. Smart Location Scoring (+40 points)
      if (locationQuery) {
        const loc = locationQuery.toLowerCase();
        let expandedLocations = [loc];

        Object.keys(SMART_DICTIONARY.locations).forEach(key => {
          if (loc.includes(key) || key.includes(loc)) {
            // @ts-ignore
            expandedLocations = [...expandedLocations, ...SMART_DICTIONARY.locations[key]];
          }
        });

        if (expandedLocations.some(l => job.location.toLowerCase().includes(l))) {
          score += 40;
        }
      }

      // 5. Personal Skill Match (+5 points per matching skill)
      const personalMatches = job.skills.filter(tag =>
        studentProfile.skills.some(skill => skill.toLowerCase() === tag.toLowerCase())
      ).length;
      score += (personalMatches * 5);

      // If they are searching, and this job got 0 points, hide it completely (-1)
      if (isSearching && score === 0) return { ...job, matchScore: -1 };

      // If they ARE NOT searching, give everything a base score of 1 so it shows up
      if (!isSearching && score === 0) score = 1;

      return { ...job, matchScore: score };

    })
      .filter(job => job.matchScore !== -1) // Remove hidden jobs
      .sort((a, b) => {
        // SORT 1: Highest Score at the Top! (This pushes partial matches to the bottom)
        // @ts-ignore
        if (b.matchScore !== a.matchScore) return b.matchScore - a.matchScore;

        // SORT 2: Push applied jobs to the bottom
        ...
        // SORT 3: Push viewed jobs below unviewed jobs
        const aViewed = viewedJobs.has(a.id) ? 1 : 0;
        const bViewed = viewedJobs.has(b.id) ? 1 : 0;
        return aViewed - bViewed;
      });

  }, [allJobs, studentProfile, activeTab, searchQuery, locationQuery, categoryQuery, appliedJobs, viewedJobs]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="relative group">
            ...
            <input
              ...
              placeholder="Category or Title (e.g. Web Design)..."
              className="w-full pl-12 pr-4 py-3.5 text-sm font-medium bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all dark:text-white"
            />
          </div>
            ...
            {displayJobs.map((job, index) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mt-4 shrink-0">
                  ...
                  {/* Top Match indicator based on weighted score positioning */}
                  {!appliedJobs.has(job.id) && index < 2 && (searchQuery || categoryQuery || locationQuery) && (
