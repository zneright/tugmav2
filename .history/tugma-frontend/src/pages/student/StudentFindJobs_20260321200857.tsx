...
export default function FindJobs() {
  ...
  const handleAiSearch = async () => {
    ...
    try {
      ...
      alert(error.message || "Failed to connect to AI Engine.");
    } finally {
      setIsAiSearching(false);
    }
  };
    ...
    <div className="space-y-6 pb-20 md:pb-8 animate-in fade-in duration-500 relative max-w-5xl mx-auto">
      ...
      {jobToApply && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-md shadow-2xl border border-zinc-200 dark:border-zinc-800 relative overflow-hidden flex flex-col mx-auto max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              <div className="flex items-center gap-3 mb-4 mt-2">
                ...
                <h3 className="text-lg sm:text-xl font-black text-zinc-900 dark:text-white leading-tight">Data Consent <br /><span className="text-[13px] sm:text-sm font-medium text-zinc-500">Submit Application</span></h3>
              </div>
            ...
            {/* Sticky Footer */}
            <div className="p-4 sm:p-6 border-t border-zinc-100 dark:border-zinc-800 flex flex-col sm:flex-row gap-3 sm:justify-end shrink-0 bg-white dark:bg-zinc-900">
              <button
                onClick={() => setJobToApply(null)}
                disabled={isApplying}
                className="w-full sm:w-auto px-5 py-3 sm:py-2.5 rounded-xl text-sm font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isApplying}
                className="w-full sm:w-auto px-5 py-3 sm:py-2.5 rounded-xl text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 shadow-lg shadow-purple-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 active:scale-95"
              >
      ...
      {selectedJob && studentProfile && (
        <div className="fixed inset-0 z-[50] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          {/* 🔥 MODAL CONTAINER: flex-col, max-h-[90vh], overflow-hidden to keep footer sticky 🔥 */}
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-lg max-h-[90vh] shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col mx-auto overflow-hidden">
            ...
            <div className="overflow-y-auto custom-scrollbar flex-1 relative">
              ...
              <div className="px-4 sm:px-6 pb-6 -mt-8 relative z-10">
                <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-lg border border-zinc-100 dark:border-zinc-800 mb-5 sm:mb-6">
                  <h2 className="text-lg sm:text-xl font-black text-zinc-900 dark:text-white leading-tight">{selectedJob.title}</h2>

                  <button
                    onClick={() => { setSelectedJob(null); setSelectedEmployerUid(selectedJob.firebase_uid); setViewMode('company'); }}
                    className="text-zinc-500 dark:text-zinc-400 font-medium text-sm flex items-center gap-1.5 mt-1 hover:text-purple-600 dark:hover:text-purple-400 transition-colors group"
                  >
                  ...
                  {selectedJob.aiReason && (
                    <div className="mt-3 p-3 sm:p-2 bg-purple-50 dark:bg-purple-500/10 rounded-lg border border-purple-100 dark:border-purple-500/20 text-xs font-medium text-purple-700 dark:text-purple-300 flex items-start gap-1.5 leading-relaxed">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-5 sm:mb-6">
                  <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-700">
                    ...
                    <p className="font-bold flex items-center gap-1.5 text-zinc-900 dark:text-white text-sm sm:text-base">
                  ...
                  <div className="bg-purple-50 dark:bg-purple-500/10 p-4 rounded-2xl border border-purple-100 dark:border-purple-500/20 shadow-sm shadow-purple-500/5">
                    <p className="text-[11px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                      <Sparkles size={12} className="shrink-0" /> Est. End Date
                    </p>
                    ...
                    <p className="text-[10px] font-medium text-purple-600/70 dark:text-purple-400/70 mt-1 leading-tight">
                      Based on your {studentProfile.required_hours} hrs
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-2 sm:mb-6 border-t border-zinc-100 dark:border-zinc-800 pt-5 sm:pt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(selectedJob.skills || []).map(tag => {
                      ...
                      return (
                        <span key={tag} className={`text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 ${isMatch
            ...
            {/* 🔥 STICKY FOOTER 🔥 */}
            <div className="p-4 sm:p-6 bg-white dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800 shrink-0 z-20">
              {appliedJobs.has(Number(selectedJob.id)) ? (
                ...
                <button onClick={() => handleInitiateApplication(selectedJob)} className="w-full py-3.5 rounded-xl font-black text-white bg-purple-600 hover:bg-purple-700 shadow-lg shadow-purple-500/20 transition-all flex items-center justify-center gap-2 active:scale-95">
                  Submit Application
                </button>
              )}
            </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-4 sm:p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          <h1 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white flex items-center gap-2 tracking-tight">
            Smart Job Matchmaker
          </h1>
          <button
            onClick={handleAiSearch}
            disabled={isAiSearching}
            className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white px-5 py-3 sm:py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-purple-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="relative group w-full">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-purple-600 transition-colors" size={16} />
            <input
              type="text"
              value={searchQuery}
              ...
              className="w-full pl-10 pr-4 py-3 sm:py-3.5 text-sm font-medium bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all dark:text-white"
            />
          </div>

          <div className="relative group w-full">
            <Tag className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-purple-600 transition-colors" size={16} />
            <input
              type="text"
              value={categoryQuery}
          ...
          <div className="relative group w-full">
            <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-purple-600 transition-colors" size={16} />
            <input
              type="text"
              value={locationQuery}
        ...
        {/* Tab Filters */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide pt-2 w-full">
      ...
      <div className="space-y-4">
        ...
        {isAiSearching ? (
          <div className="text-center py-20 bg-white/50 dark:bg-zinc-900/50 rounded-3xl border border-purple-200 dark:border-purple-900/50 backdrop-blur-sm mx-4 sm:mx-0">
          ...
          <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800 mx-4 sm:mx-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {displayJobs.map((job) => (
              <div
                ...
                className={`bg-white dark:bg-zinc-900 rounded-3xl border p-4 sm:p-5 shadow-sm transition-all group cursor-pointer relative overflow-hidden flex flex-col ${appliedJobs.has(job.id)
              ...
              >
                ...
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="text-base sm:text-lg font-black text-zinc-900 dark:text-white leading-tight sm:group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors truncate">
                        {job.title}
                      </h3>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-y-2 gap-x-2 sm:gap-x-3 mt-4 text-[10px] sm:text-[11px] font-bold text-zinc-600 dark:text-zinc-300">
                  <span className="flex items-center gap-1.5 bg-zinc-100 dark:bg-zinc-800 px-2.5 py-1.5 rounded-lg shrink-0">
                  ...
                  <span className={`px-2.5 py-1.5 rounded-lg shrink-0 ${job.salary !== 'Unpaid / Volunteer' ? 'text-emerald-700 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-500/10' : 'text-zinc-500 bg-zinc-100 dark:bg-zinc-800'}`}>
                ...
                <div className="flex flex-wrap gap-1.5 mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
                  {(job.skills || []).slice(0, 4).map(tag => {
                    const isMatch = studentProfile?.skills.some(skill => skill.toLowerCase() === tag.toLowerCase());
                    return (
                      <span key={tag} className={`text-[9px] sm:text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-md ${isMatch
                        ? 'bg-purple-100 text-purple-700 border border-purple-200 dark:bg-purple-500/20 dark:border-purple-500/30 dark:text-purple-300'
                ...
                {job.aiReason && (
                  <div className="mt-4 text-[11px] sm:text-xs font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/10 p-2 sm:p-2.5 rounded-lg border border-purple-100 dark:border-purple-800/30 line-clamp-2">
