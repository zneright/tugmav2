...
export default function ConnectedEmployers() {
  ...
  // Add this function inside your ConnectedEmployers component
  ...
  // --- Update the applications.map inside the JSX ---
  {
      ...
      <div key={app.id} className={`p-4 rounded-2xl border transition-all ${app.status === 'Offered'
        ...
        {app.status === 'Offered' && (
          <div className="mt-4 flex gap-2">
            <button
              onClick={() => handleOfferAction(app.id, employer.company_name, 'Hired')}
              className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-xl transition-all shadow-md active:scale-95"
            >
              Accept Offer
            </button>
            <button
              onClick={() => handleOfferAction(app.id, employer.company_name, 'Rejected')}
              className="px-4 py-2 bg-white border border-zinc-200 text-zinc-600 hover:text-red-600 hover:bg-red-50 rounded-xl text-xs font-bold transition-all active:scale-95"
            >
              Decline
            </button>
          </div>
            ...
            return (
              <div key={employer.employer_uid} className={`bg-white dark:bg-zinc-900 border ${isHired ? 'border-emerald-200 dark:border-emerald-800/50 shadow-emerald-500/5' : 'border-zinc-200 dark:border-zinc-800'} rounded-3xl p-6 shadow-sm hover:shadow-lg transition-all group flex flex-col relative overflow-hidden`}>
                ...
                <div className="space-y-2 mb-8 flex-1">
                  ...
                  {employer.applications.map((app) => (
                    <div key={app.id} className={`p-3 rounded-2xl border ${app.status === 'Hired' ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-800/30' : 'bg-zinc-50 dark:bg-zinc-800/30 border-zinc-100 dark:border-zinc-800'}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2 min-w-0">
                          <Briefcase size={14} className={`${app.status === 'Hired' ? 'text-emerald-600 dark:text-emerald-400' : 'text-purple-500'} shrink-0 mt-0.5`} />
                          <span className={`text-sm font-bold truncate leading-tight ${app.status === 'Hired' ? 'text-emerald-950 dark:text-emerald-50' : 'text-zinc-700 dark:text-zinc-300'}`}>
                            {app.role}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-2">
                        <span className={`text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider ${app.status === 'Hired' ? 'bg-emerald-200 text-emerald-800 dark:bg-emerald-500/30 dark:text-emerald-300' :
                          app.status === 'Shortlisted' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' :
                            app.status === 'Rejected' ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300' :
