...
import { Search, MapPin, Filter, Clock, Building2, ChevronRight, CheckCircle2, Eye, Calendar, Sparkles, AlertCircle, Loader2, Tag, X } from 'lucide-react';
...
export default function FindJobs() {
  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // 👇 MASSIVE FIX FOR HOURS AND SKILLS 👇
        // 1. Safely extract OJT Required Hours
        let parsedHours = 450; // Default
        if (data.ojt) {
          const ojtObj = typeof data.ojt === 'string' ? JSON.parse(data.ojt) : data.ojt;
          if (ojtObj && ojtObj.requiredHours) {
            parsedHours = Number(ojtObj.requiredHours);
          }
        }

        // 2. Safely extract Skills
  ...
  const calculateEndDate = (startDate: string, daysPerWeek: number, hoursPerDay: number) => {
    ...
    // Start on the exact day the company specified
    let currentDate = new Date(startDate);

    // Cap days per week at 7 to prevent infinite loops from bad database entries
    const workDays = Math.min(daysPerWeek, 7);

    let failsafe = 0; // Prevents browser crash just in case

    // Step through the calendar day-by-day until hours reach 0
    while (remainingHours > 0 && failsafe < 2000) {
      ...
      // If workDays is 5, valid working days are 1 (Mon) through 5 (Fri).
      // If workDays is 3, valid working days are 1 (Mon) through 3 (Wed).
      const currentDayNumber = dayOfWeek === 0 ? 7 : dayOfWeek;

      // If today is a working day, subtract the daily hours
      if (currentDayNumber <= workDays) {
        remainingHours -= hoursPerDay;
      }

      // If we still have hours left to complete, move to tomorrow
      if (remainingHours > 0) {
  ...
  }, [allJobs, aiJobs, activeTab, appliedJobs, viewedJobs]);
