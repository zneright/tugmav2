...
export default function StudentProfile() {
  ...
  const handleResumeUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // We set the actual FILE object into state, not just base64
      setProfileData(prev => ({
        ...prev,
        resumeName: file.name,
        resumeFile: file // 👈 Add this temporary property to your state
      }));
    }
  };

  // 2. Updated Save Profile Logic (Switching to FormData)
  const handleSaveProfile = async () => {
    const user = auth.currentUser;
    if (!user) return;
    ...
    try {
      ...
      // Append Photos (if they are new base64 strings)
      formData.append('profilePhoto', profileData.profilePhoto);
      formData.append('coverPhoto', profileData.coverPhoto);

      // 🔥 THE REAL FILE UPLOAD 🔥
      if ((profileData as any).resumeFile) {
        formData.append('resume', (profileData as any).resumeFile);
      }

      const response = await fetch(`http://localhost:8080/api/users/profile/${user.uid}`, {
        method: 'POST', // CodeIgniter prefers POST for File Uploads
        body: formData,
        // ❌ REMOVE 'Content-Type': 'application/json' (Browser sets it automatically for FormData)
      });
