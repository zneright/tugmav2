...
import { Search, MapPin, Filter, Clock, Building2, ChevronRight, CheckCircle2, Eye, Calendar, Sparkles, AlertCircle, Loader2, Tag, X, ShieldAlert, FileWarning, Briefcase, Lock } from 'lucide-react';
...
interface Job {
  ...
  matchScore?: number;
  aiReason?: string;
  status?: string; // Added to track active/closed state
}
...
export default function FindJobs() {
  ...
  const fetchAllActiveJobs = async () => {
    try {
      // Changed endpoint if you want to pull all jobs including closed, 
      // or ensure your backend sends the status with the active jobs.
  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      const roleRes = await fetch(`http://localhost:8080/api/users/role/${firebaseUid}`);
      let userStatus = 'active';
      if (roleRes.ok) {
        const roleData = await roleRes.json();
        userStatus = roleData.status?.toLowerCase() || 'active';
      }
  ...
  const handleAiSearch = async () => {
    ...
    try {
      ...
      const searchTerms = [searchQuery, locationQuery, categoryQuery].filter(Boolean).join(' | ');
      logSystemEvent('Used AI Matchmaker', `Searched jobs using terms: [${searchTerms}]`);
  ...
  const handleInitiateApplication = (job: Job) => {
    ...
    if (job.status?.toLowerCase() === 'closed') {
      showToast("This job posting is already closed and is no longer accepting applications.", "error");
      return;
    }
  ...
  const confirmAndSubmitApplication = async () => {
    ...
    try {
      ...
      logSystemEvent('Submitted Job Application', `Applied for "${jobToApply.title}" at ${jobToApply.companyName || 'Unknown Company'}`);
    ...
    <div className="space-y-5 sm:space-y-6 pb-20 md:pb-8 animate-in fade-in duration-500 relative max-w-5xl mx-auto">
      ...
      {jobToApply && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-950 rounded-3xl w-full max-w-md shadow-2xl border border-zinc-200 dark:border-zinc-800 relative overflow-hidden flex flex-col mx-auto max-h-[85vh]">
            <div className="absolute top-0 left-0 w-full h-2 bg-purple-500 z-50" />

            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-white dark:bg-zinc-950 overscroll-none">
              <div className="flex items-center gap-3 mb-5 mt-2">
                ...
                <div>
                  ...
                  <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Submit Application</span>
                </div>
              </div>
            ...
            <div className="p-4 sm:p-6 border-t border-zinc-100 dark:border-zinc-800 flex flex-col sm:flex-row gap-3 sm:justify-end shrink-0 bg-white dark:bg-zinc-950 z-20">
              <button
      ...
      {selectedJob && studentProfile && (
        <div className="fixed inset-0 z-[50] flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-3 sm:p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-950 rounded-[2rem] w-full max-w-lg max-h-[85vh] shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col mx-auto overflow-hidden relative">

            <div className="overflow-y-auto custom-scrollbar flex-1 relative bg-white dark:bg-zinc-950 overscroll-none pb-6">
            ...
            <div className="p-4 sm:p-6 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800 shrink-0 z-20 sticky bottom-0">
              {/* REPLACED "ALREADY HIRED" CHECK WITH "CLOSED JOB" CHECK */}
              {selectedJob.status?.toLowerCase() === 'closed' ? (
                <button disabled className="w-full py-4 sm:py-3.5 rounded-xl font-black text-white bg-zinc-300 dark:bg-zinc-800 flex items-center justify-center gap-2 cursor-not-allowed text-zinc-500">
                  <Lock size={18} /> Job Closed
                </button>
            ...
            {displayJobs.map((job) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-zinc-900 rounded-[1.5rem] border p-4 sm:p-5 shadow-sm transition-all group cursor-pointer relative overflow-hidden flex flex-col ${isClosed ? 'opacity-70 grayscale-[30%] border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50'
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 mt-4 shrink-0">
                    ...
                    {job.matchScore !== undefined && !isClosed && (
