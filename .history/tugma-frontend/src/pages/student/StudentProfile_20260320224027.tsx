...
export default function StudentProfile() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    resumeName: "",
    resumeFile: null as File | null, // 👈 NEW: Holds the actual physical file
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Arrays and Objects must be stringified for FormData
      ...
      if (profileData.coverPhoto) formData.append('coverPhoto', profileData.coverPhoto);

      // Append the actual physical file!
      if (profileData.resumeFile) {
        formData.append('resume', profileData.resumeFile);
        formData.append('resumeName', profileData.resumeFile.name);
      }

      // ⚠️ Note: We use POST here because CodeIgniter prefers POST for handling multipart/form-data
      const response = await fetch(`http://localhost:8080/api/users/profile/${user.uid}`, {
        method: 'POST', 
        body: formData // No Content-Type header needed; the browser sets it automatically for FormData
      });
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-3 p-4 bg-purple-50 dark:bg-purple-500/10 border border-purple-100 dark:border-purple-500/20 rounded-xl mb-4">
              ...
              {profileData.resumeName && !isEditing && (
                <a
                  href={`http://localhost:8080/uploads/resumes/${profileData.resumeName}`}
                  target="_blank"
                  rel="noreferrer"
