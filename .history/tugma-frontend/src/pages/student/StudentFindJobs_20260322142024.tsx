...
interface StudentProfile {
  ...
  course: string;
  resume_name: string;
  status?: string; // Need status to check if hired
}
...
export default function FindJobs() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStudentProfile({
          ...
          status: userStatus // Save status for hire check
        });
      }
    } catch (error) {
  ...
  const handleInitiateApplication = (job: Job) => {
    // Prevent hired students from applying
    if (studentProfile?.status === 'hired') {
      showToast("You are already hired for an OJT position. You cannot apply for new roles.", "error");
      return;
    }
        ...
        <div className="fixed inset-0 z-[50] flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-3 sm:p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-950 rounded-[2rem] w-full max-w-lg max-h-[85vh] shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col mx-auto overflow-hidden relative">
            ...
            <div className="p-4 sm:p-6 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-800 shrink-0 z-20 sticky bottom-0">
              {studentProfile?.status === 'hired' ? (
                <button disabled className="w-full py-4 sm:py-3.5 rounded-xl font-black text-white bg-zinc-400 dark:bg-zinc-700 flex items-center justify-center gap-2 cursor-not-allowed">
                  <Briefcase size={18} /> Already Hired
                </button>
