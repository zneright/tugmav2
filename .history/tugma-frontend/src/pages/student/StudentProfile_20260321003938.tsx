...
export default function StudentProfile() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    about: "",
    resumeName: "",
    resumeData: "", // Now stores the Cloudinary URL
    profilePhoto: "", // Now stores the Cloudinary URL
    coverPhoto: "", // Now stores the Cloudinary URL
    education: {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Cloudinary standard upload limit is usually around 10MB, capping at 5MB is safe
    ...
    try {
      ...
      setProfileData(prev => ({
        ...
        resumeData: resumeUrl // Store the clean Cloudinary URL!
      }));
    } catch (error) {
      // Error handled in the helper
    }
  ...
  const handleViewResume = () => {
    if (profileData.resumeData) {
      // It's a real URL now, so we can just open it!
