  ...
  const handleOfferAction = async (applicationId: number, companyName: string, action: 'Hired' | 'Rejected') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 🔥 LOG THE EVENT 🔥
        const actionText = action === 'Hired' ? 'Accepted Job Offer' : 'Declined Job Offer';
        const detailsText = action === 'Hired'
          ? `Accepted offer from ${companyName} and officially started OJT.`
          : `Declined offer from ${companyName}.`;
        logSystemEvent(actionText, detailsText);
      }
    } catch (error) {
