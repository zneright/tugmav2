...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    hired: 0,
    reviewed: 0,
    total_hours: 0 // Pulled from DTR Logs table
  });
  ...
  // 🔥 NEW: Check if the user has started logging hours
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                {/* 🔥 DYNAMIC: Sign MOA & Documents 🔥 */}
                <div className={`flex items-start gap-4 p-4 rounded-2xl transition-all ${hasStartedLogging ? 'bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 opacity-60' : 'border border-zinc-200 dark:border-zinc-800'}`}>
                  {hasStartedLogging ? <CheckCircle2 className="text-emerald-500 shrink-0 mt-0.5" size={24} /> : <div className="w-6 h-6 rounded-full border-2 border-zinc-300 dark:border-zinc-700 shrink-0 mt-0.5"></div>}
                  <div>
                    <h4 className={`font-bold ${hasStartedLogging ? 'text-emerald-900 dark:text-emerald-400 line-through' : 'text-zinc-900 dark:text-white'}`}>Sign MOA & Documents</h4>
                    <p className={`text-xs mt-1 ${hasStartedLogging ? 'text-emerald-700 dark:text-emerald-500/80' : 'text-zinc-500 dark:text-zinc-400'}`}>Coordinate with your school admin.</p>
                  </div>
                </div>

                {/* 🔥 DYNAMIC: Daily Log Entry 🔥 */}
                <div className={`flex items-start gap-4 p-4 rounded-2xl transition-all ${hasStartedLogging ? 'bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 opacity-60' : 'border border-zinc-200 dark:border-zinc-800'}`}>
                  {hasStartedLogging ? <CheckCircle2 className="text-emerald-500 shrink-0 mt-0.5" size={24} /> : <div className="w-6 h-6 rounded-full border-2 border-zinc-300 dark:border-zinc-700 shrink-0 mt-0.5"></div>}
                  <div>
                    <h4 className={`font-bold ${hasStartedLogging ? 'text-emerald-900 dark:text-emerald-400 line-through' : 'text-zinc-900 dark:text-white'}`}>Daily Log Entry</h4>
                    <p className={`text-xs mt-1 ${hasStartedLogging ? 'text-emerald-700 dark:text-emerald-500/80' : 'text-zinc-500 dark:text-zinc-400'}`}>Record your hours in the workspace.</p>
                  </div>
                </div>
