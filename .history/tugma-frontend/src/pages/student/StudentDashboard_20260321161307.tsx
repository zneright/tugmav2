...
export default function Dashboard() {
  ...
  const fetchStats = async (userUid: string) => {
    try {
      const res = await fetch(`http://localhost:8080/api/interactions/dashboard/${userUid}`);
      if (res.ok) {
        const data = await res.json();

        // Update your Stat Cards with REAL data
        setStatCards([
          { title: 'Applied', value: data.applied.toString(), icon: <Briefcase size={20} />, color: 'text-blue-600', bg: 'bg-blue-50' },
          { title: 'Profile Views', value: data.viewed.toString(), icon: <Eye size={20} />, color: 'text-purple-600', bg: 'bg-purple-50' },
          { title: 'Pending', value: data.pending.toString(), icon: <Clock size={20} />, color: 'text-amber-600', bg: 'bg-amber-50' },
          { title: 'Shortlisted', value: data.shortlisted.toString(), icon: <Star size={20} />, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        ]);

        // Update OJT Progress State
        setOjtProgress({
          completed: data.total_hours,
          required: data.required_hours,
          status: data.hired > 0 ? 'In Progress' : 'Pending'
        });
      }
    } catch (error) {
