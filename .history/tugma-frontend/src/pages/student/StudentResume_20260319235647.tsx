...
import {
  ...
  ListChecks, Target, XCircle, SearchCheck, CheckCircle
} from 'lucide-react';
...
export default function Resume() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'application/pdf') {
      if (file.size > 8 * 1024 * 1024) {
        alert("File is too large! Please upload a PDF that is 8MB or below.");
        return;
      }
  ...
  const processResumeAPI = async () => {
    ...
    try {
      ...
      alert("Failed to scan resume. Make sure your backend is running.");
      setIsScanning(false);
    }
  };
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            ...
            {!selectedFile ? (
              ...
              >
                ...
                <p className="text-[11px] font-medium text-zinc-500">Max file size: 8MB</p>
              </div>
            ) : (
          ...
          {!isScanned && !isScanning ? (
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex flex-col items-center justify-center relative shrink-0">
                  <svg className="w-40 h-40 transform -rotate-90 transition-all duration-1000 ease-out">
                    ...
                    <circle
                      ...
                      className={feedback.match_score >= 80 ? 'text-emerald-500' : feedback.match_score >= 60 ? 'text-amber-500' : 'text-red-500'}
                      style={{ transition: 'stroke-dashoffset 1.5s ease-out' }}
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-5xl font-black tracking-tight ${feedback.match_score >= 80 ? 'text-emerald-600 dark:text-emerald-400' : feedback.match_score >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                ...
                {/* Sub Metrics based on new JSON schema */}
                <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { label: 'Experience Match', score: feedback.experience_relevance },
                    { label: 'Education Match', score: feedback.education_relevance },
                    { label: 'Skills Match', score: feedback.skills_relevance }
                  ].map((metric) => (
                    <div key={metric.label} className="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700/50">
                      ...
                      <div className="flex items-end justify-between mb-2">
                        <span className={`text-2xl font-black leading-none ${metric.score >= 80 ? 'text-emerald-500' : metric.score >= 60 ? 'text-amber-500' : 'text-red-500'}`}>
                          {metric.score}/100
                        </span>
                      </div>
                      <div className="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-1.5 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${metric.score >= 80 ? 'bg-emerald-500' : metric.score >= 60 ? 'bg-amber-500' : 'bg-red-500'}`}
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
                ...
                <div className="p-6 space-y-6">
                  ...
                  {/* Overall Assessment */}
                  <div className="bg-purple-50/50 dark:bg-purple-500/5 border border-purple-100 dark:border-purple-500/10 rounded-xl p-4">
                    <h3 className="text-[11px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider mb-2">Overall Assessment</h3>
                    <p className="text-[13px] text-zinc-700 dark:text-zinc-300 font-medium leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <div>
                      <h3 className="text-sm font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-2 mb-3">
                        <SearchCheck size={16} /> Matched Keywords
                      </h3>
                      <div className="flex flex-wrap gap-2">
                        {feedback.matched_keywords.length > 0 ? feedback.matched_keywords.map(kw => (
                          <span key={kw} className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-[11px] font-black rounded-lg uppercase tracking-wider">
                            {kw}
                          </span>
                        )) : (
                          <span className="text-[12px] text-zinc-500">No matching keywords found.</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {feedback.missing_keywords.length > 0 ? feedback.missing_keywords.map(kw => (
                          ...
                          <span className="text-[12px] text-zinc-500">No missing keywords!</span>
                        )}
                      </div>
                    </div>
                  </div>
