...
import OJTTracker from './OJTTracker'; // Or wherever you saved it
...
export default function StudentProfile() {
  ...
  const [modalImage, setModalImage] = useState<string | null>(null); // --- ADDED MODAL STATE ---
  ...
  const [profileData, setProfileData] = useState({
    ...
    coverPhoto: "",   // Base64 Image String
  ...
  // --- HANDLERS: IMAGE UPLOADS (Converted to Base64 for Hackathon ease) ---
    ...
    <div className="space-y-6 pb-20 md:pb-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
        ...
        <div
          ...
          onClick={() => profileData.coverPhoto && setModalImage(profileData.coverPhoto)} // --- ADDED ONCLICK ---
        >
          {isEditing && (
            <button
              onClick={(e) => {
                e.stopPropagation(); // --- PREVENT MODAL OPENING WHEN CLICKING CAMERA ---
          ...
          <div className="flex justify-between items-end -mt-12 sm:-mt-16 mb-6 relative z-10">
            ...
            <div className="relative group">
              <div
                ...
                onClick={() => profileData.profilePhoto && setModalImage(profileData.profilePhoto)} // --- ADDED ONCLICK ---
              >
              ...
              {isEditing ? (
                <div className="relative">
                  <select
                    ...
                    className="bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 text-xs font-bold pl-4 pr-10 py-2.5 rounded-lg border border-green-200 dark:border-green-900/50 outline-none appearance-none cursor-pointer shadow-sm"
                  >
                ...
                <span className="bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400 text-[11px] uppercase tracking-wider font-bold px-4 py-1.5 rounded-full border border-green-200 dark:border-green-900/50 w-fit">
                  {profileData.ojt.status}
                </span>
              )}
            </div>

            <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
              {isEditing ? (
                <div className="flex items-center gap-6">
                  <div className="flex-1">
                    <label className="text-[11px] text-zinc-500 font-bold uppercase tracking-wider mb-2 block">Completed Hours</label>
                    <input type="number" value={profileData.ojt.completedHours} onChange={(e) => setProfileData({ ...profileData, ojt: { ...profileData.ojt, completedHours: parseInt(e.target.value) || 0 } })} className="w-full px-4 py-3 bg-white dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700/80 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all text-sm font-bold text-zinc-900 dark:text-white shadow-sm" />
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <input type="number" value={profileData.ojt.requiredHours} onChange={(e) => setProfileData({ ...profileData, ojt: { ...profileData.ojt, requiredHours: parseInt(e.target.value) || 0 } })} className="w-full px-4 py-3 bg-white dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700/80 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all text-sm font-bold text-zinc-900 dark:text-white shadow-sm" />
                  </div>
                </div>
              ) : (
                <>
                  <div className="flex justify-between items-end mb-3">
                    <div>
                      <p className="text-4xl font-black text-zinc-900 dark:text-white tracking-tight">{profileData.ojt.completedHours} <span className="text-lg font-medium text-zinc-500">/ {profileData.ojt.requiredHours} hrs</span></p>
                    </div>
      ...
      {modalImage && (
        ...
        >
          ...
          <img
            ...
            onClick={(e) => e.stopPropagation()} // Keeps modal open if the image itself is clicked
          />
        </div>
      )}
    </div>
