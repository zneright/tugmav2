...
export default function FindJobs() {
  ...
  const handleJobClick = async (job: Job) => {
    ...
    // 🔥 FIX 2: Update UI state using the "Functional Update" pattern with a NEW Set
    if (!viewedJobs.has(jobId) && !appliedJobs.has(jobId)) {
      setViewedJobs(prev => {
        const next = new Set(prev); // Copy existing
        next.add(jobId);            // Add new
        return next;                // Return brand new object to trigger re-render
      });
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mt-4 shrink-0">
                  ...
                  {appliedJobs.has(Number(job.id)) ? (
                    ...
                    /* 2. If Viewed (and NOT applied): Show Grey Badge */
                  ...
                  {/* 3. AI Match Score Badge (Always show if exists) */}
