...
export default function FindJobs() {
  ...
  const confirmAndSubmitApplication = async () => {
    ...
    try {
      ...
      // This endpoint handles saving it to the database perfectly.
      const res = await fetch('http://localhost:8080/api/interactions/record', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Update local state so the "Applied" badge appears instantly
      setAppliedJobs(prev => {
        const next = new Set(prev);
      ...
      // 🔥 THIS CLOSES THE MODALS SO YOU CAN APPLY TO OTHER JOBS!
      setJobToApply(null);
      setSelectedJob(null);
