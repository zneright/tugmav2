    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              {!selectedFile ? (
                <div
                  ...
                  className="border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 hover:bg-purple-50/50 transition-all group"
                >
            ...
            <div className="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50">
              ...
              <textarea
                ...
                className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 text-[13px] font-medium outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 dark:text-white transition-all resize-none shadow-sm"
          ...
          {/* HISTORY SIDEBAR */}
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex-1 flex flex-col overflow-hidden max-h-[500px]">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
              {isLoadingHistory ? (
                ...
                <p className="text-xs text-center text-zinc-500 p-4 font-medium">No previous scans found in database.</p>
              ) : (
                history.map((item) => (
                  <button
                    key={item.id}
        ...
        <div className="lg:col-span-8">
          {isScanned && feedback ? (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="space-y-4 flex-1">
                  <p className="text-sm font-bold text-zinc-900 dark:text-white">AI Assessment</p>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={m.label} className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center shadow-sm">
                    <span className={`text-2xl font-black ${m.val >= 75 ? 'text-emerald-500' : m.val >= 60 ? 'text-amber-500' : 'text-red-500'}`}>{m.val}%</span>
                    <p className="text-[10px] font-bold uppercase text-zinc-500 mt-1">{m.label}</p>
                  </div>
                ))}
              </div>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-red-200 dark:border-red-900/30 p-6 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {feedback.missing_keywords.length > 0 ? feedback.missing_keywords.map(k => (
                      <span key={k} className="px-3 py-1.5 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-[11px] font-black rounded-lg uppercase tracking-wider">{k}</span>
                    )) : <p className="text-xs text-zinc-500">None found!</p>}
                  </div>
                </div>

                {/* Matched Keywords */}
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-emerald-200 dark:border-emerald-900/30 p-6 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {feedback.matched_keywords.length > 0 ? feedback.matched_keywords.map(k => (
                      <span key={k} className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[11px] font-black rounded-lg uppercase tracking-wider">{k}</span>
                    )) : <p className="text-xs text-zinc-500">None found.</p>}
                  </div>
                </div>

                {/* Improvements */}
                <div className="md:col-span-2 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            ...
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl text-center p-10 bg-zinc-50/50 dark:bg-zinc-900/20">
              <div className="w-20 h-20 bg-white dark:bg-zinc-800 rounded-full flex items-center justify-center mb-6 shadow-sm">
