...
export default function FindJobs() {
  ...
  const fetchAllActiveJobs = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Defensive check to ensure skills are always arrays
  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // 👇 ROBUST HOURS & SKILLS EXTRACTION 👇
        const parsedHours = Number(data.requiredHours) || Number(data.required_hours) || 450;
  ...
  const handleAiSearch = async () => {
    ...
    try {
      ...
      // 🔥 FIX: If the server returns an error (like 500), throw it so the catch block alerts us!
      if (!res.ok) {
