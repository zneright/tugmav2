...
// This mimics AI natural language understanding for instant, zero-latency matching.
const SMART_DICTIONARY = {
  locations: {
    ...
    "cebu": ["dasmarinas", "santa rosa", "binan", "carmona"]
  },
  categories: {
    "Web Development": ["react", "frontend", "backend", "fullstack", "php", "javascript", "html", "css", "node", "laravel", "codeigniter"],
    "Software Engineering": ["c#", ".net", "java", "python", "c++", "software", "desktop", "systems"],
    "UI/UX Design": ["figma", "design", "ui", "ux", "wireframe", "prototype", "photoshop", "illustrator"],
    "Data & Analytics": ["sql", "database", "data", "analytics", "excel", "python", "machine learning"],
    "IT Support & Network": ["network", "hardware", "support", "helpdesk", "troubleshooting", "cisco", "sysadmin"]
  }
};
...
export default function FindJobs() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        await Promise.all([
          fetchActiveJobs(),
          fetchStudentProfile(user.uid) // 👈 Now hitting your actual Users API
        ]);
      }
      setIsLoading(false);
    });
  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      // 👇 Hits the new Users.php getProfile route 👇
      const res = await fetch(`http://localhost:8080/api/users/profile/${firebaseUid}`);
      if (res.ok) {
        const data = await res.json();
        setStudentProfile({
          // Make sure your frontend sends 'requiredHours' when saving the student profile!
          required_hours: Number(data.requiredHours) || Number(data.required_hours) || 450,
          skills: data.skills || []
        });
      } else {
        setStudentProfile({ required_hours: 450, skills: [] }); // Fallback
      }
    } catch (error) {
  ...
  // --- 3. SMART SORTING & FILTERING ENGINE ---
  const filteredAndSortedJobs = useMemo(() => {
    ...
    return jobs.filter(job => {
      ...
      // 2. Category Filter (Smart Match)
      if (selectedCategory !== 'All Categories') {
        // @ts-ignore
        const categoryKeywords = SMART_DICTIONARY.categories[selectedCategory] || [];
        const jobText = `${job.title} ${job.skills.join(' ')}`.toLowerCase();
        // Check if any keyword from the category matches the job title or skills
        const matchesCategory = categoryKeywords.some((kw: string) => jobText.includes(kw));
      ...
      // Calculate Match Score based on database profile skills
      let score = 0;
      ...
      // Sort logic: High score first, unapplied first, unviewed first
  ...
  }, [jobs, studentProfile, activeTab, searchQuery, locationQuery, selectedCategory, viewedJobs, appliedJobs]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-lg overflow-y-auto max-h-[90vh] shadow-2xl custom-scrollbar border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="px-6 pb-6 -mt-8 relative z-10">
              ...
              {/* 👇 NEW: DYNAMIC END DATE CALCULATOR 👇 */}
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-700">
                  <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Possible Start Date</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="relative group">
            ...
            >
              ...
              {Object.keys(SMART_DICTIONARY.categories).map(cat => (
