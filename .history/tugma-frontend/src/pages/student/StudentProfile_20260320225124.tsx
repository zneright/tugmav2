...
const PHILIPPINE_CLASSIFICATIONS = [
  ...
  "Arts, Design & Multimedia", "Aviation & Aeronautics", "Business, Management & Administration",
  "Communication, Media & Broadcasting", "Education & Teacher Training", "Engineering (Civil, Mechanical, EE, CE, etc.)",
  "Healthcare & Allied Medicine (Nursing, MedTech, etc.)", "Hospitality, Tourism & Culinary Arts",
  "Human Resources & Psychology", "Information Technology & Computer Science", "Law, Public Administration & Criminal Justice",
  "Manufacturing, Logistics & Supply Chain", "Maritime Studies", "Science & Mathematics", "Social Sciences & Humanities"
];
...
export default function StudentProfile() {
          ...
          try {
            ...
            if (response.ok) {
              ...
              setProfileData(prev => ({
                ...
                skills: data.skills || [], languages: data.languages || [], preferredLocations: data.preferredLocations || []
              }));
            }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const fd = new FormData(); // Renamed to 'fd' to avoid confusion with your state
      fd.append('firstName', profileData.firstName);
      fd.append('lastName', profileData.lastName);
      fd.append('title', profileData.title);
      fd.append('address', profileData.address);
      fd.append('classification', profileData.classification);
      fd.append('course', profileData.course);
      fd.append('expectedSalary', profileData.expectedSalary);
      fd.append('about', profileData.about);

      // Stringify arrays/objects for PHP to decode
      fd.append('education', JSON.stringify(profileData.education));
      fd.append('ojt', JSON.stringify(profileData.ojt));
      fd.append('skills', JSON.stringify(profileData.skills));
      fd.append('languages', JSON.stringify(profileData.languages));
      fd.append('preferredLocations', JSON.stringify(profileData.preferredLocations));

      // Append real files
      if (profileData.profilePhotoFile) fd.append('profilePhotoFile', profileData.profilePhotoFile);
      if (profileData.coverPhotoFile) fd.append('coverPhotoFile', profileData.coverPhotoFile);
      if (profileData.resumeFile) fd.append('resume', profileData.resumeFile);

      // 🔥 USE POST, as PUT doesn't handle FormData files well in PHP
      const response = await fetch(`http://localhost:8080/api/users/profile/${user.uid}`, {
        method: 'POST',
        body: fd
      });
      ...
      window.location.reload(); // Refresh to show actual server URLs
    } catch (error) {
      console.error("Error saving profile:", error);
      alert("Failed to save profile. Check if the server is running and file sizes are okay.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleRemoveTag = (type: 'skills' | 'languages' | 'preferredLocations', indexToRemove: number) => {
    setProfileData({ ...profileData, [type]: profileData[type].filter((_, index) => index !== indexToRemove) });
  };

  if (isLoading) return <div className="h-96 flex items-center justify-center text-white"><Loader2 className="animate-spin text-purple-500" size={40} /></div>;
    ...
    <div className="space-y-6 pb-20 md:pb-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
        <div
          ...
          style={{ backgroundImage: profileData.coverPhoto ? `url(${profileData.coverPhoto})` : 'linear-gradient(to right, #9333ea, #8b5cf6, #4f46e5)' }}
          onClick={() => profileData.coverPhoto && !isEditing && setModalImage(profileData.coverPhoto)}
        >
          {isEditing && (
            <button onClick={(e) => { e.stopPropagation(); coverPhotoRef.current?.click(); }} className="absolute top-4 right-4 p-2.5 bg-black/40 hover:bg-black/60 text-white rounded-full backdrop-blur-md transition-colors shadow-lg">
              <Camera size={18} />
            </button>
          )}
        </div>
          ...
          <div className="flex justify-between items-end -mt-12 sm:-mt-16 mb-6 relative z-10">
            <div className="relative group">
              ...
              >
                ...
                {isEditing && (
                  <button onClick={(e) => { e.stopPropagation(); profilePhotoRef.current?.click(); }} className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
            ...
            <div className="flex gap-2">
              {isEditing ? (
                <button onClick={handleSaveProfile} disabled={isSaving} className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-sm shadow-emerald-500/20 flex items-center gap-2 transition-all disabled:opacity-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl">
                ...
                <div className="space-y-4">
                  <div className="relative w-full">
                    ...
                    <select value={profileData.classification} onChange={(e) => setProfileData({ ...profileData, classification: e.target.value })} className={selectClass}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            {isEditing ? (
              <div className="max-w-xs relative">
                ...
                <input
                  type="number" value={profileData.ojt.requiredHours}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            {isEditing ? (
              <textarea
                rows={5} value={profileData.about}
              ...
              {[
                ...
                <div key={edu.key} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                  ...
                  <div className={`w-[calc(100%-3rem)] md:w-[calc(50%-2rem)] bg-white dark:bg-zinc-900 p-5 rounded-2xl border ${edu.border} hover:shadow-lg dark:hover:shadow-zinc-950/50 transition-all`}>
                    ...
                    {isEditing ? (
                      <input
                        type="text" value={(profileData.education as any)[edu.key]}
