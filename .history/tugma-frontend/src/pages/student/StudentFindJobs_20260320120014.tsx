...
export default function FindJobs() {
  ...
  const [allJobs, setAllJobs] = useState<Job[]>([]); // Default jobs from DB
  const [aiJobs, setAiJobs] = useState<Job[] | null>(null); // Jobs returned by AI Search
  ...
  // --- SEARCH STATES ---
  const [activeTab, setActiveTab] = useState('All'); // Remote, Hybrid, Onsite, Paid
  ...
  const handleAiSearch = async () => {
    ...
    setAiJobs(null); // Clear previous AI results

    // If search inputs are empty, reset to show all jobs
  ...
  const displayJobs = useMemo(() => {
    // Determine the source pool
    let baseJobs = aiJobs !== null ? aiJobs : allJobs;

    // Apply the hardware Filter Tab (Remote, Hybrid, Paid)
    ...
    return finalJobs.sort((a, b) => {
      // 1. If AI scored them, prioritize highest score
      if (a.matchScore && b.matchScore && a.matchScore !== b.matchScore) {
        return b.matchScore - a.matchScore;
      }

      // 2. Push Applied jobs to bottom
      ...
      // 3. Push Viewed jobs below unviewed
