...
export default function StudentProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      const user = auth.currentUser; // Or use your unsubscribe logic
      if (user) {
        try {
          ...
          if (response.ok) {
            ...
            setProfileData(prev => ({
              ...prev,
              ...data,
              // Map DB snake_case back to React camelCase if necessary
              ...
              education: {
                ...
                college: data.education?.college || { school: data.school || "", years: "" },
              }
            }));
          }
        } catch (error) {
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-zinc-700 before:to-transparent">
              {[
                ...
                <div key={edu.key} className="relative z-10">
                  <p className="text-[11px] font-bold uppercase mb-2">{edu.label}</p>
                  {isEditing ? (
                    <div className="space-y-2">
                      <input
                        type="text"
                        value={profileData.education[edu.key as keyof typeof profileData.education]?.school || ""}
                        onChange={(e) => setProfileData({
                          ...profileData,
                          education: {
                            ...profileData.education,
                            [edu.key]: { ...profileData.education[edu.key as keyof typeof profileData.education], school: e.target.value }
                          }
                        })}
                      ...
                      <input
                        type="text"
                        value={profileData.education[edu.key as keyof typeof profileData.education]?.years || ""}
                        onChange={(e) => setProfileData({
                          ...profileData,
                          education: {
                            ...profileData.education,
                            [edu.key]: { ...profileData.education[edu.key as keyof typeof profileData.education], years: e.target.value }
                          }
                        })}
                    ...
                    <div>
                      <p className="text-sm font-bold text-zinc-900 dark:text-white">
                        {profileData.education[edu.key as keyof typeof profileData.education]?.school || "No school set"}
                      </p>
                      <p className="text-xs text-zinc-500">
                        {profileData.education[edu.key as keyof typeof profileData.education]?.years || ""}
                      </p>
                    </div>
                  )}
                </div>
