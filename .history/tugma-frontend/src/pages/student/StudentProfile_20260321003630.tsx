...
export default function StudentProfile() {
      ...
      const unsubscribe = auth.onAuthStateChanged(async (user) => {
        if (user) {
          try {
            ...
            // Wait for the JSON response so we can read error messages from CodeIgniter
            const data = await response.json();

            // If the status is 400, 404, 500, etc., throw an error!
            if (!response.ok) {
              throw new Error(data.messages?.error || data.message || `Server Error: ${response.status} ${response.statusText}`);
            }

            // If successful, populate the state
            setProfileData(prev => ({
              ...prev,
              ...data,
            ...
            // Show the exact error in an alert so you aren't guessing
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Parse the response so we can read the actual error message
      const result = await response.json();

      if (!response.ok) {
        // Throw the specific error message provided by CodeIgniter
      ...
      // Now the alert will tell us exactly what MySQL is complaining about!
