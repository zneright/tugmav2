...
export default function OJTTrackerPage() {
    ...
    const handleDownloadPDF = () => {
        ...
        doc.setTextColor(100, 50, 200); // Sleek Purple
        doc.text("DAILY TIME RECORD", 105, 20, { align: "center" });

        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(100, 100, 100);
        doc.text("Official OJT Hours Log", 105, 26, { align: "center" });
        ...
        doc.setFont("helvetica", "bold");
        doc.text("Completed:", 130, 49);
        doc.setFont("helvetica", "normal");
        doc.text(`${completedHours} Hours`, 155, 49);

        // 3. Log Table (Removed the Double Pay Column, added Row Highlighting)
        ...
        autoTable(doc, {
            ...
            body: tableRows,
            theme: 'grid',
            headStyles: { fillColor: [100, 50, 200], textColor: 255, halign: 'left' },
            columnStyles: {
                0: { cellWidth: 30 },
                1: { cellWidth: 'auto' },
            ...
            // 🔥 This highlights the row light orange if it was a double-pay day
            didParseCell: function (data) {
                if (data.section === 'body') {
                    // Check our original logs array to see if this specific row is double pay
                    const isDouble = logs[data.row.index].isDouble;
                    if (isDouble) {
                        data.cell.styles.fillColor = [255, 240, 225]; // Light Orange Highlight

                        // Add a small note to the Task column
                        if (data.column.index === 1) {
                            data.cell.text += " (Holiday/Overtime Credit)";
                        }
                    }
                }
            }
        ...
        // 4. Supervisor Signature Section
        ...
        doc.setFontSize(10);
        doc.setTextColor(100, 100, 100);
        doc.text("I hereby certify that the above records are true and correct.", 14, sigY - 10);

        doc.setFont("helvetica", "bold");
        doc.setFontSize(11);
        doc.setTextColor(30, 30, 30);
        doc.text("Certified Correct By:", 14, sigY + 10);

        doc.setDrawColor(50, 50, 50);
        doc.line(14, sigY + 30, 90, sigY + 30); // Signature Line
        doc.setFont("helvetica", "normal");
        doc.text("Supervisor Signature over Printed Name", 14, sigY + 36);

        doc.line(140, sigY + 30, 190, sigY + 30); // Date Line
        doc.text("Date", 140, sigY + 36);
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className="className="flex items-center gap-2 bg-purple-100 hover:bg-purple-200 text-purple-700 dark:bg-purple-500/20 dark:hover:bg-purple-500/30 dark:text-purple-300 px-4 py-2 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 shadow-sm""
                            >
