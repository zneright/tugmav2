...
// Helper to convert the File object into the Base64 format Gemini needs
...
export default function StudentResume() {
  ...
  const processResumeAPI = async () => {
    ...
    try {
      // 1. Initialize the Google SDK directly in React
      const genAI = new GoogleGenerativeAI('AIzaSyAJQBYDfVr6KKx_yx8S6TY7heYfMm7gh7A'); // Paste your key here
      const model = genAI.getGenerativeModel({
        model: "gemini-1.5-flash",
        generationConfig: { responseMimeType: "application/json" } // Force JSON output!
      });

      // 2. Prepare the PDF and the Prompt
      const pdfPart = await fileToGenerativePart(selectedFile);

      const prompt = `YOU ARE A WORLD-CLASS APPLICANT TRACKING SYSTEM (ATS). ANALYZE THE ATTACHED RESUME PDF AGAINST THE FOLLOWING JOB DESCRIPTION.
      ...
      // 4. Parse the JSON and update state
      ...
      alert(error.message || "Failed to scan. Please try again.");
    } finally {
      setIsScanning(false);
    }
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {feedback.matched_keywords.map(kw => (
