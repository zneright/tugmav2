...
export default function StudentProfile() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    about: "",
    resumeName: "",
    resumeData: "", // 👈 NEW: Stores the actual file content as Base64
    profilePhoto: "",
    coverPhoto: "",
    education: {
            ...
            if (response.ok) {
              ...
              setProfileData(prev => ({
                ...
                education: {
                  ...
                  college: data.education?.college || { school: "", years: "" },
                },
  ...
  // 👇 NEW: Resume Upload Logic to Base64 👇
  const handleResumeUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      // Keep files under 2MB for local Base64 storage to prevent DB crashing
      ...
      reader.onloadend = () => {
        setProfileData(prev => ({
          ...
          resumeData: reader.result as string // Save the Base64 string
        }));
      };
      reader.readAsDataURL(file);
    }
  ...
  const handleViewResume = () => {
    if (profileData.resumeData) {
      // Open the Base64 string in a new browser tab!
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-zinc-700 before:to-transparent">
              {[
                ...
                <div key={edu.key} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                  {/* Timeline Dot */}
                  <div className="flex items-center justify-center w-6 h-6 rounded-full border-4 border-white dark:border-zinc-900 bg-purple-500 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow z-10"></div>

                  {/* Content Card */}
                  <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2rem)] bg-zinc-50 dark:bg-zinc-800/40 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-700 hover:shadow-md transition-all">
                    <p className="text-[11px] font-bold uppercase tracking-wider mb-2 text-purple-500">{edu.label}</p>
                    ...
                    {isEditing ? (
                      <div className="space-y-3">
                        <input
                          ...
                          className="w-full text-[13px] font-semibold bg-transparent border-b border-zinc-300 dark:border-zinc-600 outline-none focus:border-purple-500 transition-colors text-zinc-900 dark:text-white"
                        />
                        <input
                          type="text"
                          ...
                          className="w-full text-[11px] bg-transparent border-b border-zinc-200 dark:border-zinc-700 outline-none text-zinc-500 font-medium"
                        />
                      </div>
                    ) : (
                      <>
                        <p className="text-sm font-bold text-zinc-900 dark:text-white">
                          {(profileData.education as any)[edu.key]?.school || "Not Specified"}
                        </p>
                        <p className="text-xs font-medium text-zinc-500 mt-1">
                          {(profileData.education as any)[edu.key]?.years || "Years not set"}
                        </p>
                      </>
                    )}
                  </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-zinc-700 before:to-transparent">
              ...
              {[
                ...
                <div key={edu.key} className="...">
                  <p className="text-[11px] font-bold uppercase mb-2">{edu.label}</p>
                  {isEditing ? (
                    <div className="space-y-2">
                      <input
                        type="text"
                        value={profileData.education[edu.key]?.school || ""}
                        onChange={(e) => setProfileData({
                          ...profileData,
                          education: {
                            ...profileData.education,
                            [edu.key]: { ...profileData.education[edu.key], school: e.target.value }
                          }
                        })}
                        placeholder="School Name"
                        className="w-full text-[13px] font-semibold bg-transparent border-b border-zinc-300 outline-none"
                      />
                      <input
                        type="text"
                        value={profileData.education[edu.key]?.years || ""}
                        onChange={(e) => setProfileData({
                          ...profileData,
                          education: {
                            ...profileData.education,
                            [edu.key]: { ...profileData.education[edu.key], years: e.target.value }
                          }
                        })}
                        placeholder="Years (e.g. 2022 - 2026)"
                        className="w-full text-[11px] bg-transparent border-b border-zinc-200 outline-none text-zinc-500"
                      />
                    </div>
                  ) : (
                    <div>
                      <p className="text-sm font-bold text-zinc-900 dark:text-white">
                        {profileData.education[edu.key]?.school || "No school set"}
                      </p>
                      <p className="text-xs text-zinc-500">
                        {profileData.education[edu.key]?.years || ""}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                  ...
                  <div className="flex flex-col gap-3 p-4 bg-purple-50 dark:bg-purple-500/10 border border-purple-100 dark:border-purple-500/20 rounded-xl mb-4">
                    ...
                    {/* Added a View Button so the student can double-check what they uploaded! */}
