...
export default function FindJobs() {
  ...
  const fetchInteractions = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // 🔥 FIX: Map the array to Numbers just in case the API sent strings
        const viewedIds = (data.viewed || []).map((id: any) => Number(id));
        const appliedIds = (data.applied || []).map((id: any) => Number(id));

        setViewedJobs(new Set(viewedIds));
  ...
  const handleJobClick = async (job: Job) => {
    ...
    // 1. Check if we already have this as 'viewed' or 'applied'
