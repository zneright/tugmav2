  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Because Users.php merges 'profile_data', the keys might be nested or named differently.

        let parsedHours = 450; // Default
        if (data.requiredHours) parsedHours = Number(data.requiredHours);
        if (data.required_hours) parsedHours = Number(data.required_hours);
