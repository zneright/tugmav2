...
export default function FindJobs() {
  ...
  const confirmAndSubmitApplication = async () => {
    ...
    // 🛑 SECONDARY PREVENT: Just in case
    ...
    try {
      // 1. Record interaction as 'applied'
      const recordRes = await fetch('http://localhost:8080/api/interactions/record', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          student_uid: uid,
      ...
      // 2. Submit formal application to the employer
      ...
      if (!applyRes.ok) {
        ...
        // If backend already has this application, it will return a 409 or error message
        throw new Error(errorData.messages?.error || "Application submission failed.");
      }
