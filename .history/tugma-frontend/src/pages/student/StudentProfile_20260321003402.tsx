...
const PHILIPPINE_CLASSIFICATIONS = [
  ...
  // ... Keep your other classifications
];

// 👇 REPLACE THESE WITH YOUR ACTUAL CLOUDINARY DETAILS 👇
const CLOUDINARY_UPLOAD_PRESET = "YOUR_UNSIGNED_UPLOAD_PRESET"; // e.g., "student_uploads"
...
export default function StudentProfile() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    about: "",
    resumeName: "",
    resumeData: "", // This will now store the Cloudinary URL!
  ...
  // ... Keep your useEffect fetchProfile exactly the same ...
  useEffect(() => {
    // [Your existing fetchProfile code remains unchanged]
    setIsLoading(false); // Placeholder so the code runs
  }, []);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.messages?.error || result.message || "Unknown Server Error");
  ...
  // 👇 NEW: Generic Cloudinary Upload Helper 👇
  const uploadToCloudinary = async (file: File): Promise<string> => {
    ...
    try {
      // 'auto' allows Cloudinary to figure out if it's an image or a raw document (like PDF)
      ...
      return data.secure_url; // Returns the secure HTTPS URL from Cloudinary
    } catch (error) {
  ...
  // 👇 REWRITTEN: Upload images directly to Cloudinary 👇
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'profilePhoto' | 'coverPhoto') => {
    ...
    try {
      ...
      // Error handled inside upload helper
    }
  };

  // 👇 REWRITTEN: Upload Resumes directly to Cloudinary 👇
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Optional: Keep file size limits (Cloudinary free tier allows 10MB per file)
    if (file.size > 5000000) {
      alert("File is too large. Please upload a PDF or image smaller than 5MB.");
      return;
    }
    ...
    try {
      ...
      setProfileData(prev => ({
        ...
        resumeData: resumeUrl // Now storing the clean URL instead of massive Base64
      }));
    } catch (error) {
      // Error handled inside upload helper
    }
  ...
  const handleViewResume = () => {
    if (profileData.resumeData) {
      // Because it's a real URL now, we can just open it in a new tab!
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 dark:before:via-zinc-700 before:to-transparent">
              ...
              {[
                ...
                // Safely grab the current education object, or default to empty strings
