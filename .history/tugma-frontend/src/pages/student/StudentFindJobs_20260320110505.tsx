...
import { Search, MapPin, Filter, Clock, Building2, ChevronRight, CheckCircle2, Eye, Calendar, Sparkles, AlertCircle, Loader2 } from 'lucide-react';
import { auth } from '../../firebaseConfig';
import { onAuthStateChanged } from 'firebase/auth';

// --- SMART LOCATION DICTIONARY (AI-like localized search) ---
...
interface Job {
  ...
  description: string;
  skills: string[];
  companyName?: string; // Will be mapped if you have employer info
}
...
export default function FindJobs() {
  ...
  // Status Tracking (Local state for Hackathon UX flow)
  ...
  const fetchStudentProfile = async (firebaseUid: string) => {
    try {
      const res = await fetch(`http://localhost:8080/api/student/profile/${firebaseUid}`);
      if (res.ok) {
        const data = await res.json();
        setStudentProfile({
          required_hours: data.required_hours || 450, // Default to 450 if not set
          skills: data.skills || []
        });
      } else {
        // Fallback if student hasn't set up profile yet
  ...
  // --- 3. THE SMART SORTING & FILTERING ENGINE ---
  const filteredAndSortedJobs = useMemo(() => {
    if (!studentProfile) return []; // Don't sort until profile loads

    return jobs.filter(job => {
      // 1. Tab Filter
      if (activeTab !== 'All' && job.work_setup !== activeTab && job.salary === 'Unpaid / Volunteer' && activeTab === 'Paid Allowance') return false;
      if (activeTab === 'Paid Allowance' && job.salary === 'Unpaid / Volunteer') return false;
      if (['Remote', 'Hybrid', 'Onsite'].includes(activeTab) && job.work_setup !== activeTab) return false;

      // 2. Keyword Search
      const searchLower = searchQuery.toLowerCase();
      const matchesSearch = !searchQuery ||
        job.title.toLowerCase().includes(searchLower) ||
        job.skills.some(tag => tag.toLowerCase().includes(searchLower));
      ...
      // 4. Calculate Match Score dynamically from the DB profile!
      let score = 0;
      job.skills.forEach(tag => {
        if (studentProfile.skills.some(skill => skill.toLowerCase() === tag.toLowerCase())) {
          score += 1;
        }
      });
  ...
  }, [jobs, studentProfile, activeTab, searchQuery, locationQuery, viewedJobs, appliedJobs]);
  ...
  const handleApply = async (jobId: number) => {
    // In a real app, this would POST to /api/applications
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-lg overflow-y-auto max-h-[90vh] shadow-2xl custom-scrollbar border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="h-32 w-full relative">
              <img src={selectedJob.image_url} alt="Cover" className="w-full h-full object-cover" />
            ...
            <div className="px-6 pb-6 -mt-8 relative z-10">
              <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-lg border border-zinc-100 dark:border-zinc-800 mb-6">
                ...
                <p className="text-zinc-500 dark:text-zinc-400 font-medium flex items-center gap-1 mt-1">
                  <Building2 size={14} /> {selectedJob.companyName || "Employer"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl">
                  ...
                  <p className="font-bold flex items-center gap-1.5 text-zinc-900 dark:text-white"><Calendar size={14} className="text-purple-500" /> {new Date(selectedJob.start_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                </div>
                <div className="bg-purple-50 dark:bg-purple-500/10 p-4 rounded-2xl border border-purple-100 dark:border-purple-500/20">
                  ...
                  <p className="text-[10px] text-purple-600/70 dark:text-purple-400/70 mt-1 leading-tight">Based on your {studentProfile.required_hours}hr requirement</p>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 group">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 text-sm font-medium bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 transition-all dark:text-white shadow-sm inset-shadow-sm"
            />
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-2">
          <p className="text-sm font-bold text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
            Showing <span className="text-purple-600 bg-purple-50 dark:bg-purple-500/10 px-2 py-0.5 rounded-md">{filteredAndSortedJobs.length}</span> active matches
          </p>
        </div>
        ...
        {filteredAndSortedJobs.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800">
            ...
            <h3 className="font-bold text-lg text-zinc-900 dark:text-white">No active jobs found</h3>
            ...
            {filteredAndSortedJobs.map((job) => (
              ...
              >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="w-16 h-16 rounded-2xl overflow-hidden shrink-0 border border-zinc-100 dark:border-zinc-800 relative">
                    <img src={job.image_url} alt="Cover" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
