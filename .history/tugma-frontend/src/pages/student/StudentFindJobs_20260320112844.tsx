...
export default function FindJobs() {
  ...
  const [allJobs, setAllJobs] = useState<Job[]>([]); // Holds all active jobs initially
  const [aiJobs, setAiJobs] = useState<Job[] | null>(null); // Holds jobs returned by AI
  ...
  const handleAiSearch = async () => {
    ...
    // If they click search but all fields are empty, just reset to all jobs
    if (!searchQuery && !locationQuery && selectedCategory === 'All Categories') {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAiJobs(aiMatchedJobs); // Store the AI results specifically!
      }
    } catch (error) {
  ...
  const displayJobs = useMemo(() => {
    // 1. Decide which pool of jobs to use: AI results, or all jobs
    let baseJobs = aiJobs !== null ? aiJobs : allJobs;

    // 2. Apply ONLY the top Tab filter (All, Remote, Hybrid, Paid)
    ...
    // 3. Sort them properly!
    return finalJobs.sort((a, b) => {
      // Rule 1: Highest AI Score first (if we have AI scores)
      if (a.matchScore && b.matchScore && a.matchScore !== b.matchScore) {
        return b.matchScore - a.matchScore;
      }

      // Rule 2: Push Applied jobs to the bottom
      ...
      // Rule 3: Push Viewed jobs below unviewed jobs
