...
export default function FindJobs() {
  ...
  const fetchInteractions = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // 🔥 FIX 1: Create brand new Sets from the incoming data
  ...
  const handleJobClick = async (job: Job) => {
    ...
    if (!viewedJobs.has(jobId) && !appliedJobs.has(jobId)) {
      // 1. Instantly show the badge in UI
      setViewedJobs(prev => {
      ...
      // 2. Safely tell the database
      try {
        const res = await fetch('http://localhost:8080/api/interactions/record', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ student_uid: uid, job_id: jobId, type: 'viewed' })
        });

        if (!res.ok) {
          const errData = await res.json();
          console.error("API Error saving view:", errData);
        }
      } catch (error) {
        console.error("Network Error recording view:", error);
      }
    }
  ...
  const confirmAndSubmitApplication = async () => {
    ...
    if (appliedJobs.has(jobToApply.id)) {
      setJobToApply(null);
      return;
    }
    ...
    try {
      // 🔥 THE FIX: We ONLY call interactions/record now. No double-submitting!
      const res = await fetch('http://localhost:8080/api/interactions/record', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.messages?.error || errorData.message || "Submission failed.");
      }

      // Force React to show the green "Applied" badge instantly
      setAppliedJobs(prev => {
        const next = new Set(prev);
  ...
  const confirmAndSubmitApplication = async () => {
    ...
    // Prevent UI glitches if they click fast
    if (appliedJobs.has(jobToApply.id)) {
      setJobToApply(null);
      return;
    }
    ...
    try {
      // 1. Record interaction as 'applied' in job_interactions table
      await fetch('http://localhost:8080/api/interactions/record', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // 2. Formal submit to the employer's dashboard
      ...
      if (!applyRes.ok) throw new Error("Submission failed.");

      // 3. Update local state so badge appears instantly
      const updatedApplied = new Set(appliedJobs);
      updatedApplied.add(jobToApply.id);
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mt-4 shrink-0">
                  ...
                  {appliedJobs.has(Number(job.id)) ? (
                    ...
                    /* Priority 2: Viewed Badge (Only if NOT applied) */
