...
interface StudentProfile {
  ...
  course: string;
  resume_name: string; // 👈 Needed for Resume Check
}
...
export default function FindJobs() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        await Promise.all([
          ...
          fetchInteractions(user.uid) // 👈 Pulls DB history!
        ]);
      }
      setIsLoading(false);
    });
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStudentProfile({
          ...
          course: data.course || '',
          resume_name: data.resumeName || '' // 👈 Grab resume state
        });
      }
    } catch (error) {
  ...
  const displayJobs = useMemo(() => {
    ...
    return finalJobs.sort((a, b) => {
      // 1. AI Score (If searching)
      if (a.matchScore && b.matchScore && a.matchScore !== b.matchScore) {
        return b.matchScore - a.matchScore;
      }

      // 2. Put Applied jobs at the absolute bottom
      ...
      if (!isAiSearching && aiJobs === null && studentProfile) {
        ...
        // Check Classification & Course matches in Job Title/Description (15 pts)
        ...
        // Sort by highest default score
        if (aScore !== bScore) return bScore - aScore;
      }

      // 5. Tie-breaker: Most recent jobs first
      return b.id - a.id;
    });
  }, [allJobs, aiJobs, activeTab, appliedJobs, viewedJobs, studentProfile, isAiSearching]);

  const handleJobClick = (job: Job) => {
    // Add to Local UI state instantly
    setViewedJobs(prev => new Set(prev).add(job.id));
    // Save to Database permanently
    recordInteraction(job.id, 'viewed');
    setSelectedJob(job);
  };

  // 👇 RESUME CHECK INITIATOR 👇
