...
export default function Auth() {
  ...
  const handleSocialLogin = async (provider: any, role: AuthView) => {
    ...
    try {
      ...
      // 2. Check if user already exists in CodeIgniter MySQL
      ...
      if (roleResponse.ok) {
        ...
        const payload = {
          firebase_uid: user.uid,
          email: user.email,
          role: role, // Default to the view they are currently on
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        // User closed the window, ignore error
      } else {
