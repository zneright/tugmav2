...
import {
  ...
  MapPin, Edit2, Eye, X, ImageIcon, Check, Loader2, CalendarClock, Trash2, Clock, MonitorSmartphone
} from 'lucide-react';
...
export default function EmployerJobs() {
  ...
  const [companyLocation, setCompanyLocation] = useState<string>('Company HQ');
  ...
  const [newLocation, setNewLocation] = useState(''); // 👈 NEW: For custom location tags
  ...
  const [formData, setFormData] = useState({
    ...
    salary: '',
    locationSelection: '',
    customLocations: [] as string[], // 👈 NEW: Array for multiple locations
    days_per_week: '5',
    hours_per_day: '8',
    description: '',
    skills: [] as string[]
  ...
  const fetchJobs = async (firebaseUid: string) => {
    ...
    } catch (error) { console.error("Failed to fetch jobs", error); } finally { setIsLoading(false); }
  };
  ...
  const fetchEmployerProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const loc = data.location || 'Company HQ';
        setCompanyLocation(loc);
        setFormData(prev => ({ ...prev, locationSelection: loc }));
      }
  ...
  const handleDeleteJob = async (jobId: number) => {
    ...
    try {
      ...
      if (res.ok) { if (uid) fetchJobs(uid); showToast('Draft deleted successfully!', 'success'); }
      else { const err = await res.json(); showToast("Failed to delete: " + (err.messages?.error || err.message), 'error'); }
  ...
  const openCreateModal = () => {
    ...
    setFormData({
      ...
      locationSelection: companyLocation, customLocations: [], // Reset custom locations
  ...
  const openEditModal = (job: Job) => {
    ...
    const isHQ = job.location === companyLocation;
    
    // 👇 If not HQ, split the comma-separated string back into an array 👇
    const customLocArray = isHQ ? [] : job.location.split(',').map(l => l.trim()).filter(l => l);
    ...
    setFormData({
      ...
      locationSelection: isHQ ? companyLocation : 'Other',
  ...
  const openViewModal = (job: Job) => {
    ...
    setFormData({
      ...
      locationSelection: isHQ ? companyLocation : 'Other', customLocations: customLocArray,
  ...
  const handleSaveJob = async (status: 'Active' | 'Draft' | 'Closed') => {
    ...
    // 👇 Convert array of locations to a single comma-separated string for the DB 👇
    const finalLocation = formData.locationSelection === 'Other' 
      ? formData.customLocations.join(', ') 
      : formData.locationSelection;

    if (!formData.title.trim() || !finalLocation || !formData.description.trim() || formData.skills.length === 0) {
      showToast("Please fill in all required fields, including at least one skill and location!", 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        else if (status === 'Closed') showToast('Posting closed successfully.');
        else if (modalMode === 'edit') showToast('Job updates published successfully!');
  ...
  const addLocation = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && newLocation.trim() !== '') {
      e.preventDefault();
      if (!formData.customLocations.includes(newLocation.trim())) {
        setFormData({ ...formData, customLocations: [...formData.customLocations, newLocation.trim()] });
      }
      setNewLocation('');
    }
  };
  const removeLocation = (locationToRemove: string) => {
    setFormData({ ...formData, customLocations: formData.customLocations.filter(l => l !== locationToRemove) });
  };
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <select disabled={isReadOnly} name="work_setup" value={formData.work_setup} onChange={handleChange} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 outline-none transition-all dark:text-white text-sm font-medium appearance-none disabled:opacity-70">
                ...
                {/* 👇 DYNAMIC LOCATION LOGIC WITH MULTIPLE TAGS 👇 */}
                <div className="space-y-3 sm:col-span-2 bg-zinc-50 dark:bg-zinc-800/30 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                  <label className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                    <MapPin size={14} className="text-purple-500" /> Office Location <span className="text-red-500">*</span>
                  </label>
                  <div className="flex flex-col gap-3 mt-2">
                    <select disabled={isReadOnly} name="locationSelection" value={formData.locationSelection} onChange={handleChange} className="w-full px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 text-sm font-medium appearance-none disabled:opacity-70">
                      <option value={companyLocation}>{companyLocation} (Company HQ)</option>
                      <option value="Other">Other Locations / Multiple Branches</option>
                    </select>

                    {formData.locationSelection === 'Other' && (
                      <div className="space-y-1.5 animate-in fade-in">
                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider ml-1">Custom Locations {!isReadOnly && "(Tap Enter to add)"}</label>
                        <div className={`w-full min-h-[50px] p-2 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-2 transition-all ${isReadOnly ? 'opacity-70' : 'focus-within:ring-2 focus-within:ring-purple-500/30 focus-within:border-purple-500'}`}>
                          {formData.customLocations.map(loc => (
                            <span key={loc} className="flex items-center gap-1.5 px-3 py-1 bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 rounded-lg text-xs font-bold shadow-sm border border-zinc-200 dark:border-zinc-700">
                              {loc} {!isReadOnly && <button type="button" onClick={() => removeLocation(loc)} className="hover:text-red-500 transition-colors"><X size={12} /></button>}
                            </span>
                          ))}
                          {!isReadOnly && (
                            <input
                              ...
                              placeholder={formData.customLocations.length === 0 ? "e.g. Makati City..." : "Add another..."}
                ...
                <div className="sm:col-span-2 bg-purple-50/50 dark:bg-purple-500/5 border border-purple-100 dark:border-purple-500/20 p-5 rounded-2xl grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select disabled={isReadOnly} name="days_per_week" value={formData.days_per_week} onChange={handleChange} className="w-full px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl border border-purple-200 dark:border-purple-800 outline-none focus:ring-2 focus:ring-purple-500/30 transition-all dark:text-white text-sm font-medium appearance-none disabled:opacity-70">
          ...
          {filteredJobs.map(job => (
            <div key={job.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col sm:flex-row gap-6 hover:border-purple-200 dark:hover:border-purple-500/30">
              ...
              <div className="w-full sm:w-32 h-48 sm:h-32 rounded-2xl overflow-hidden shrink-0 border border-zinc-100 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800 relative">
                ...
                {/* Status Dot for visual flair */}
                <div className={`absolute top-3 right-3 w-3 h-3 rounded-full border-2 border-white dark:border-zinc-900 shadow-sm ${job.status === 'Active' ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.8)]' : job.status === 'Draft' ? 'bg-amber-500' : 'bg-zinc-500'}`} />
              </div>
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 ${job.status === 'Active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : job.status === 'Draft' ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                    ...
                    <span className="px-2.5 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-[11px] font-bold text-zinc-600 dark:text-zinc-300 flex items-center gap-1.5">
                      <MapPin size={12} className="text-zinc-400" /> {job.location === companyLocation ? 'HQ' : job.location}
                    </span>
                  </div>

                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span className="px-2.5 py-1 bg-purple-50 dark:bg-purple-500/10 rounded-lg text-[11px] font-bold text-purple-700 dark:text-purple-400 flex items-center gap-1.5">
                      <CalendarClock size={12} className="text-purple-400" /> {job.days_per_week}d/wk • {job.hours_per_day}h/d
                    </span>
                    <span className="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg text-[11px] font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                      <span className="font-black">₱</span> {job.salary}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-5 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    {job.status === 'Draft' && (
                      <button onClick={() => handleDeleteJob(job.id)} className="p-2.5 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-colors" title="Delete Draft">
                        <Trash2 size={16} />
                      </button>
                    )}
