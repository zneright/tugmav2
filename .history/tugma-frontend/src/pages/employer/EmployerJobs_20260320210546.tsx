...
export default function EmployerJobs() {
  ...
  const [newLocation, setNewLocation] = useState(''); // 👈 For the new Location Tag UI
  ...
  const [formData, setFormData] = useState({
    ...
    deadline: '',
    start_date: '',
    locations: [] as string[], // Replaces the old dropdown logic
    days_per_week: '5',
    hours_per_day: '8',
    description: '',
    skills: [] as string[]
  ...
  const fetchEmployerProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setCompanyLocation(data.location || 'Company HQ');
      }
  ...
  const openCreateModal = () => {
    ...
    setFormData({
      ...
      deadline: '', start_date: '',
      locations: [companyLocation], // Pre-fill with their HQ to be helpful
  ...
  const handleSaveJob = async (status: 'Active' | 'Draft' | 'Closed') => {
    ...
    try {
      ...
      // Send whatever they typed, or blank if empty
                  ...
                  <div className={`w-full min-h-[52px] p-2 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-2 transition-all ${isReadOnly ? 'opacity-70' : 'focus-within:ring-2 focus-within:ring-purple-500/30 focus-within:border-purple-500'}`}>
                    ...
                    {!isReadOnly && (
                      <input
                        ...
                        placeholder={formData.locations.length === 0 ? "e.g. Makati City HQ..." : "Add another location..."}
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    ...
                    <span className="px-2.5 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-[11px] font-bold text-zinc-600 dark:text-zinc-300 flex items-center gap-1.5 truncate max-w-[120px]">
                      <MapPin size={12} className="text-zinc-400 shrink-0" /> {job.location === companyLocation ? 'HQ' : job.location}
                    </span>
                  </div>
