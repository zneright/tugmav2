...
import {
  ...
  MapPin, DollarSign, Edit2, Eye, X, ImageIcon, Check, Loader2
} from 'lucide-react';
import { auth } from '../../firebaseConfig'; // ⚠️ Check this path
  ...
  const handleCreateJob = async (status: 'Active' | 'Draft') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ title: '', type: 'Full-time', salary: '', location: '', description: '', skills: [] }); // Reset form
        fetchJobs(uid); // Refresh list
      } else {
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input name="salary" value={formData.salary} onChange={handleChange} type="text" placeholder="e.g. $50k - $70k" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all dark:text-white text-sm" />
                </div>
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Location</label>
                  <input name="location" value={formData.location} onChange={handleChange} type="text" placeholder="e.g. Remote, or New York, NY" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all dark:text-white text-sm" />
                </div>
              </div>
