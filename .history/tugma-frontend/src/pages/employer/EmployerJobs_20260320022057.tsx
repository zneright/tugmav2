...
import {
  ...
  MapPin, Edit2, Eye, X, ImageIcon, Check, Loader2, CalendarClock
} from 'lucide-react';
...
export default function EmployerJobs() {
  ...
  const openEditModal = (job: Job) => {
    ...
    setFormData({
      title: job.title, salary: job.salary, location: job.location,
  ...
  const handleSaveJob = async (status: 'Active' | 'Draft') => {
    ...
    try {
      ...
      // 👇 If salary is empty, set it to Unpaid 👇
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <input disabled={isReadOnly} name="salary" value={formData.salary} onChange={handleChange} type="text" placeholder="e.g. ₱500/day (Leave empty if Unpaid)" className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all dark:text-white text-sm disabled:opacity-70" />
                </div>
              </div>
              ...
              {/* Skills Tags */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Required Skills {!isReadOnly && "(Press Enter to add)"}</label>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                  ...
                  <span className="flex items-center gap-0.5 font-bold">
                    <span className="text-[13px]">₱</span> {job.salary}
                  </span>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  <div className="flex justify-between items-start">
                    <h3 className="font-black text-zinc-900 group-hover:text-purple-600 transition-colors">{job.title}</h3>
                    <div className="flex gap-1">
                      {/* View is always available */}
                      <button onClick={() => openModal('view', job)} className="p-2 hover:bg-purple-50 text-zinc-400 hover:text-purple-600" title="View Details">
                        <Eye size={16} />
                      </button>

                      {/* Edit is available unless Closed */}
                      {job.status !== 'Closed' && (
                        <button onClick={() => openModal('edit', job)} className="p-2 hover:bg-purple-50 text-zinc-400 hover:text-purple-600" title="Edit">
                          <Edit2 size={14} />
                        </button>
                      )}

                      {/* 🔥 DELETE RULE: Only show trash if NOT Active */}
                      {job.status !== 'Active' ? (
                        <button onClick={() => handleDelete(job.id)} className="p-2 hover:bg-red-50 text-zinc-400 hover:text-red-600" title="Delete Listing">
                          <Trash2 size={14} />
                        </button>
                      ) : (
                        <div className="p-2 text-zinc-300" title="Active jobs cannot be deleted">
