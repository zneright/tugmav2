    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          company_size: data.company_size || '1-10', // 👈 Added
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 fade-in pb-10">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Toggle Edit/Save */}
        {isEditing ? (
          <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition-all bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white shadow-emerald-500/20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="space-y-6">
              <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800/80 space-y-4">
                <h3 className="font-bold text-zinc-900 dark:text-white mb-4">Contact & Details</h3>

                {/* 👇 NEW COMPANY SIZE FIELD IN SIDEBAR 👇 */}
                <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                  <div className="w-8 h-8 rounded-lg bg-white dark:bg-zinc-800 flex items-center justify-center shrink-0 shadow-sm"><Users size={14} className="text-emerald-500" /></div>
                  {isEditing ? (
                    <select
                      name="company_size"
                      value={formData.company_size}
                      onChange={handleChange}
                      className="bg-transparent border-b border-zinc-300 dark:border-zinc-600 outline-none w-full pb-1 text-zinc-900 dark:text-white"
                    >
                ...
                <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                  ...
                  {isEditing ? (
                    <input name="location" value={formData.location} onChange={handleChange} placeholder="San Francisco, CA" className="bg-transparent border-b border-zinc-300 dark:border-zinc-600 outline-none w-full pb-1 text-zinc-900 dark:text-white" />
                ...
                <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                  ...
                  {isEditing ? (
                    <input name="website" value={formData.website} onChange={handleChange} placeholder="https://yourwebsite.com" className="bg-transparent border-b border-zinc-300 dark:border-zinc-600 outline-none w-full pb-1 text-zinc-900 dark:text-white" />
                  ) : (
                ...
                <div className="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                  ...
                  {isEditing ? (
                    <input name="email" type="email" value={formData.email} onChange={handleChange} placeholder="careers@company.com" className="bg-transparent border-b border-zinc-300 dark:border-zinc-600 outline-none w-full pb-1 text-zinc-900 dark:text-white" />
