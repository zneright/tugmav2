...
import {
  ...
  MapPin, Edit2, Eye, X, ImageIcon, Check, Loader2, CalendarClock, Trash2, Clock, MonitorSmartphone, Calendar
} from 'lucide-react';
...
export default function EmployerJobs() {
  ...
  const openCreateModal = () => {
    ...
    setFormData({
      ...
      deadline: '', start_date: '', // Reset dates
      locationSelection: companyLocation, customLocations: [],
  ...
  const handleSaveJob = async (status: 'Active' | 'Draft' | 'Closed') => {
    ...
    // 👇 Strict Validation including Dates 👇
    if (!formData.title.trim() || !finalLocation || !formData.description.trim() || formData.skills.length === 0 || !formData.deadline || !formData.start_date) {
      showToast("Please fill in all required fields, including dates and at least one skill!", 'error');
      return;
    }
    ...
    try {
      ...
      submitData.append('deadline', formData.deadline); // Send Deadline
      submitData.append('start_date', formData.start_date); // Send Start Date
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-3 sm:col-span-2 bg-zinc-50 dark:bg-zinc-800/30 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                  <label className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1.5"><MapPin size={14} className="text-purple-500" /> Office Location <span className="text-red-500">*</span></label>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <select disabled={isReadOnly} name="locationSelection" value={formData.locationSelection} onChange={handleChange} className="w-full sm:w-1/2 px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500 text-sm font-medium appearance-none disabled:opacity-70">
                    ...
                    {formData.locationSelection === 'Other' && (
                      <div className="w-full sm:w-1/2 space-y-1.5 animate-in fade-in">
                        <div className={`w-full min-h-[46px] p-2 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-2 transition-all ${isReadOnly ? 'opacity-70' : 'focus-within:ring-2 focus-within:ring-purple-500/30 focus-within:border-purple-500'}`}>
                          ...
                          {!isReadOnly && (
                            <input
                              ...
                              placeholder={formData.customLocations.length === 0 ? "e.g. Makati City (Tap Enter)" : "Add another..."}
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-3">
                    {job.deadline && (
                      <span className="flex items-center gap-1 text-[11px] font-bold text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-500/10 px-2.5 py-1 rounded-md">
                        <Calendar size={12} /> Deadline: {formatDate(job.deadline)}
                      </span>
                    )}
                    {job.start_date && (
                      <span className="flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 rounded-md">
                        <CalendarClock size={12} /> Starts: {formatDate(job.start_date)}
                      </span>
                    )}
                  </div>
                </div>
