...
export default function EmployerNotifications() {
  ...
  const handleUnsendBlast = async (e: React.MouseEvent, n: Notification) => {
    ...
    try {
      ...
      // 🔥 LOG THE EVENT 🔥
      logSystemEvent('Unsent Broadcast', `Retracted notification broadcast: "${n.title}"`);
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 🔥 LOG THE EVENT 🔥
        const targetString = jobId === 'ALL' ? 'All Applicants' : `Job ID #${jobId}`;
        const statusString = status === 'ALL' ? 'All Statuses' : status;
        logSystemEvent('Sent Broadcast Notification', `Target: [${targetString} - ${statusString}]. Title: "${notifyData.title}"`);
          ...
          filteredList.map((n) => {
            ...
            return (
              ...
              >
                ...
                {viewMode === 'inbox' && (
                  <div className={`p-3.5 rounded-2xl shrink-0 mt-1 transition-colors ${isAlert
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isAlert && viewMode === 'inbox' && (
                        <span className="text-[10px] font-black uppercase tracking-wider bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400 px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-900/50">
                          System Alert
                        </span>
                      )}
                    </div>
                ...
                <div className="flex flex-col items-center justify-between gap-4 self-stretch pt-2">
                  ...
                  {viewMode === 'inbox' ? (
                    <button onClick={() => handleDeleteInbox(n.id)} className="p-2 text-zinc-300 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-500/20 rounded-xl transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100">
                      <Trash2 size={16} />
                    </button>
                  ) : (
                    <button onClick={(e) => handleUnsendBlast(e, n)} className="p-2 text-zinc-300 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-500/20 rounded-xl transition-colors opacity-0 group-hover:opacity-100" title="Delete Blast">
                      <Trash2 size={16} />
                    </button>
                  )}
                </div>
