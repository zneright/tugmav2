...
import {
  ...
  FileText, ExternalLink, ThumbsUp, ThumbsDown, CalendarClock, X, BrainCircuit, RefreshCw
} from 'lucide-react';
...
export default function EmployerApplicants() {
      ...
      if (res.ok) {
        ...
        const safeData = data.map((app: any) => {
          ...
          return {
            ...
            ai_match_score: app.ai_match_score !== null ? Number(app.ai_match_score) : null
          };
        });
  ...
  const handleAnalyzeApplicant = async (applicationId: number) => {
    ...
    try {
      ...
      // Update UI instantly with new score
            ...
            <div className="relative">
              ...
              <div className="px-6 sm:px-10 pb-6 -mt-12 relative z-10 flex flex-col sm:flex-row gap-5 sm:items-end">
                ...
                <div className="flex-1 bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 mt-2 sm:mt-0 flex justify-between items-center">
                  ...
                  {/* Re-analyze Button inside Modal */}
                  <button onClick={() => handleAnalyzeApplicant(selectedApplicant.application_id)} disabled={analyzingId === selectedApplicant.application_id} className="p-2 text-zinc-400 hover:text-purple-600 hover:bg-purple-50 rounded-xl transition-colors">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  <div>
                    ...
                    <p className="font-bold text-zinc-900 dark:text-white truncate max-w-[120px] sm:max-w-[150px] flex items-center gap-2">
                      <FileText size={16} className="text-purple-500 shrink-0" />
                      {selectedApplicant.resume_name || "Not attached"}
                    </p>
                  </div>
                  {selectedApplicant.resume_name && (
                    <button className="px-4 py-2 bg-zinc-900 hover:bg-black text-white text-xs font-bold rounded-xl transition-colors">Download</button>
                  )}
                </div>
              </div>
                  ...
                  <div className="col-span-1 md:col-span-2 flex flex-col md:items-center">
                    ...
                    {app.ai_match_score !== null ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-purple-200 transition-colors disabled:opacity-50"
                      >
