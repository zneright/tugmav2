...
export default function EmployerJobs() {
  ...
  const [companyLocation, setCompanyLocation] = useState<string>(''); // 👈 Default to empty instead of 'Company HQ'
  ...
  const fetchEmployerProfile = async (firebaseUid: string) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        // 👇 Grab the ACTUAL address from the DB, otherwise leave it blank
        const actualAddress = data.address || data.location || '';
        setCompanyLocation(actualAddress);
      }
  ...
  const openCreateModal = () => {
    ...
    setFormData({
      ...
      deadline: '', start_date: '',
      locations: companyLocation ? [companyLocation] : [], // 👈 Pre-fill tag with ACTUAL DB address
