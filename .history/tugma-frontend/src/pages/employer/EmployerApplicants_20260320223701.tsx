...
export default function EmployerApplicants() {
  ...
  const handleAnalyzeApplicant = async (applicationId: number) => {
    ...
    try {
      ...
      setApplicants(prev => prev.map(app =>
        app.application_id === applicationId ? { ...app, ai_match_score: analysis.match_score, ai_assessment: analysis.overall_assessment } : app
      ));

      if (selectedApplicant && selectedApplicant.application_id === applicationId) {
        setSelectedApplicant(prev => prev ? { ...prev, ai_match_score: analysis.match_score, ai_assessment: analysis.overall_assessment } : null);
      }
    } catch (error: any) { alert("Failed to analyze candidate: " + error.message); } finally { setAnalyzingId(null); }
  };
  ...
  const exportToCSV = () => {
    ...
    applicants.forEach(app => {
      const row = [
        `"${app.first_name || ''}"`, `"${app.last_name || ''}"`, `"${app.email || ''}"`, `"${app.course || ''}"`, `"${app.job_title || ''}"`,
        app.ai_match_score !== null ? `${app.ai_match_score}%` : 'Unscanned', `"${app.status || 'Pending'}"`, `"${new Date(app.applied_at).toLocaleDateString()}"`
      ];
    ...
    <div className="space-y-6 fade-in pb-20 max-w-6xl mx-auto relative">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-2xl overflow-y-auto max-h-[90vh] shadow-2xl custom-scrollbar border border-zinc-200 dark:border-zinc-800">
            <div className="relative">
              ...
              <button onClick={() => setSelectedApplicant(null)} className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-black/40 text-white rounded-full backdrop-blur-md transition-colors"><X size={18} /></button>
              <div className="px-6 sm:px-10 pb-6 -mt-12 relative z-10 flex flex-col sm:flex-row gap-5 sm:items-end">
                ...
                <div className="flex-1 bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 mt-2 sm:mt-0 flex justify-between items-center">
                  <div className="min-w-0 pr-4">
                    <h2 className="text-xl font-black text-zinc-900 dark:text-white leading-tight truncate">{selectedApplicant.first_name || 'Unknown'} {selectedApplicant.last_name || 'User'}</h2>
                    <p className="text-zinc-500 dark:text-zinc-400 font-medium text-sm mt-0.5 truncate">{selectedApplicant.course || "Student"}</p>
                  </div>
            ...
            <div className="px-6 sm:px-10 pb-8 space-y-6">
              <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-6">
                <div className="flex items-center gap-3">
                  <button onClick={() => window.location.href = `mailto:${selectedApplicant.email}`} className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 font-bold text-sm rounded-xl flex items-center gap-2"><MessageSquare size={16} /> Email</button>
                  <button onClick={() => { setSelectedApplicant(null); setStudentUidToView(selectedApplicant.student_uid); setViewMode('profile'); }} className="text-sm font-bold text-purple-600 dark:text-purple-400 hover:underline flex items-center gap-1.5"><ExternalLink size={16} /> View Full Profile</button>
                </div>
              </div>
              ...
              {selectedApplicant.ai_match_score !== null ? (
                <div className="bg-purple-50 dark:bg-purple-500/5 border border-purple-200 dark:border-purple-800/30 p-5 rounded-2xl">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-sm font-black px-3 py-1 rounded-lg text-white ${selectedApplicant.ai_match_score >= 80 ? 'bg-emerald-500' : selectedApplicant.ai_match_score >= 50 ? 'bg-amber-500' : 'bg-red-500'}`}>{selectedApplicant.ai_match_score}% Match</span>
                  </div>
                  <p className="text-[13px] font-medium text-purple-800 dark:text-purple-400 leading-relaxed">{selectedApplicant.ai_assessment}</p>
                </div>
              ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  <div className="min-w-0 pr-2">
                    ...
                    <p className="font-bold text-zinc-900 dark:text-white truncate flex items-center gap-2"><FileText size={16} className="text-purple-500 shrink-0" /> <span className="truncate">{selectedApplicant.resume_name || "Not attached"}</span></p>
                  </div>

                  {/* 🔥 THE REAL DOWNLOAD LINK DIRECT TO FOLDER 🔥 */}
              ...
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(!selectedApplicant.job_skills || selectedApplicant.job_skills.length === 0) ? (
                    <span className="text-sm text-zinc-500 italic">No specific skills required.</span>
                  ) : (
            ...
            <div className="p-6 sticky bottom-0 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md border-t border-zinc-100 dark:border-zinc-800 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded-md ${selectedApplicant.status === 'Shortlisted' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' : selectedApplicant.status === 'Reviewed' ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400' : selectedApplicant.status === 'Rejected' ? 'bg-zinc-200 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400' : selectedApplicant.status === 'Hired' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400'}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-zinc-900 dark:text-white flex items-center gap-3">
            <div className="p-2.5 bg-purple-100 dark:bg-purple-500/20 rounded-xl"><Users className="text-purple-600 dark:text-purple-400" size={24} /></div> Applicant Tracking
          </h1>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm overflow-hidden mt-6">
        <div className="hidden md:grid grid-cols-12 gap-4 p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/30 text-[11px] font-black text-zinc-400 uppercase tracking-wider">
          <div className="col-span-4 pl-2">Candidate</div><div className="col-span-3">Applied Role</div><div className="col-span-2 text-center">AI Match Score</div><div className="col-span-2 text-center">Status</div><div className="col-span-1 text-right pr-2">Actions</div>
        </div>
                ...
                <div key={app.application_id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 md:p-5 items-center hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group">
                  ...
                  <div className="col-span-1 md:col-span-2 flex flex-col md:items-center">
                    ...
                    {app.ai_match_score !== null ? (
                      ...
                      <button onClick={() => handleAnalyzeApplicant(app.application_id)} disabled={analyzingId === app.application_id} className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-purple-200 transition-colors disabled:opacity-50 w-full md:w-auto">
                        {analyzingId === app.application_id ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} Scan
                      </button>
                    )}
                  </div>
                  ...
                  <div className="col-span-1 md:col-span-2 flex md:justify-center relative">
                    ...
                    <select
                      ...
                      className="text-xs font-bold px-3 py-1.5 rounded-lg appearance-none cursor-pointer outline-none text-center shadow-sm border w-full md:w-auto bg-zinc-100 text-zinc-700"
                    >
                  ...
                  <div className="col-span-1 md:col-span-1 flex items-center justify-end gap-2 mt-4 md:mt-0">
                    <button onClick={() => { setSelectedApplicant(app); if (app.status === 'New Applicant' || !app.status) handleStatusChange(app.application_id, 'Reviewed'); }} className="px-4 py-2 text-xs font-bold bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:text-purple-600 hover:border-purple-200 dark:hover:border-purple-500/50 dark:hover:text-purple-400 rounded-xl transition-colors shadow-sm w-full md:w-auto">
                      Review
                    </button>
                  </div>
                </div>
