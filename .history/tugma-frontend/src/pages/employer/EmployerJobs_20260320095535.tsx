  ...
  const handleSaveJob = async (status: 'Active' | 'Draft') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (status === 'Draft') {
          showToast('Job saved to your Drafts securely.');
        } else if (modalMode === 'edit') {
          showToast('Job updates published successfully!');
        } else {
          showToast('Job published! Students can now apply.');
        }
      } else {
        const err = await res.json();
                ...
                <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  <div className="flex items-center gap-1 text-zinc-500">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-zinc-900 dark:text-white truncate max-w-[140px] sm:max-w-none">
