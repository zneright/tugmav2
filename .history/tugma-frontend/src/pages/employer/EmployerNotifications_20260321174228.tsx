...
import {
  ...
  Filter, Search, X, Send, Users, UserCheck, Loader2, Trash2, SendHorizontal, Mail, Check
} from 'lucide-react';
...
export default function EmployerNotifications() {
  ...
  const handleUnsendBlast = async (e: React.MouseEvent, n: Notification) => {
    e.stopPropagation(); // Stop row click from opening recipient modal
  ...
  const handleSendNotification = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/notifications/recipients?sender_uid=${uid}&title=${encodeURIComponent(n.title)}&message=${encodeURIComponent(n.message)}`);
      if (res.ok) {
        consconst handleViewRecipients = async (n: Notification) => {t data = await res.json();
      ...
      <div className="space-y-3">
        {filteredList.map((n) => (
          <div
            ...
            className={`group p-5 rounded-3xl border flex gap-4 items-start transition-all cursor-pointer ${viewMode === 'sent' ? 'hover:border-purple-400' : ''
              } ${Boolean(n.is_read) || viewMode === 'sent' ? 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800' : 'bg-white dark:bg-zinc-900 border-purple-200 dark:border-purple-500/30 shadow-md shadow-purple-500/5'}`}
          >

            {viewMode === 'inbox' && (
              <div className={`p-3 rounded-2xl shrink-0 mt-1 transition-colors ${Boolean(n.is_read) ? 'bg-zinc-100 dark:bg-zinc-800/50' : 'bg-purple-100 dark:bg-purple-500/20'}`}>
                {getIcon(n.type)}
              </div>
            )}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <h3 className={`text-[15px] font-bold ${viewMode === 'sent' ? 'text-zinc-900 dark:text-white' : 'text-zinc-700 dark:text-zinc-300'}`}>
                    {n.title}
                  </h3>
                  {viewMode === 'sent' && (
