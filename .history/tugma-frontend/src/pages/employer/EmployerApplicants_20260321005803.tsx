...
interface Applicant {
  ...
  course: string;
  resume_name: string;
  resume_data: string; // 👈 NEW: The Cloudinary URL from the database
  job_title: string;
...
export default function EmployerApplicants() {
  ...
  // 👇 UPDATED AI TRIGGER WITH ERROR REVEAL 👇
  const handleAnalyzeApplicant = async (applicationId: number) => {
    ...
    try {
      ...
      // 1. Wait for the response text/JSON FIRST!
      const result = await res.json();

      // 2. If it's a 500 error, throw the EXACT message from CodeIgniter
      if (!res.ok) {
        throw new Error(result.messages?.error || result.message || result.error || "Unknown Server Error");
      }

      // 3. If successful, apply it
    ...
    <div className="space-y-6 fade-in pb-20 max-w-6xl mx-auto relative">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm overflow-hidden mt-6">
        <div className="hidden md:grid grid-cols-12 gap-4 p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/30 text-[11px] font-black text-zinc-400 uppercase tracking-wider">
          <div className="col-span-3 pl-2">Candidate</div> {/* Updated Span */}
          <div className="col-span-3">Applied Role</div>
          <div className="col-span-2 text-center">AI Match Score</div>
          <div className="col-span-2 text-center">Status</div>
          <div className="col-span-2 text-right pr-2">Actions</div> {/* Updated Span */}
        </div>
            ...
            {applicants.map((app) => {
              ...
              return (
                <div key={app.application_id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 md:p-5 items-center hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group">
                  ...
                  <div className="col-span-1 md:col-span-3 flex items-center gap-4"> {/* Updated Span */}
                  ...
                  {/* 👇 TABLE ACTION ICONS (Review & Resume View) 👇 */}
                  <div className="col-span-1 md:col-span-2 flex items-center justify-end gap-3 mt-4 md:mt-0">
                    {app.resume_data && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation(); // Prevents triggering other click events
                          handleViewResume(app.resume_data);
                        }}
                        title="View Resume"
                        className="p-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-xl transition-colors shadow-sm"
                      >
                        <FileText size={16} />
                      </button>
                    )}
