                ...
                >
                  ...
                  {activeJobs.map(job => (
                    <optgroup key={job.id} label={job.title}>
                      <option value={`${job.id}_ALL`}>All Applicants</option>
                      <option value={`${job.id}_Shortlisted`}>Only Shortlisted</option>
                      {/* 🔥 NEW: Option to send to hired interns 🔥 */}
                      <option value={`${job.id}_Hired`}>Only Hired (Active Interns)</option>
                    </optgroup>
                  ))}
                </select>
              </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-purple-500/20 transition-all shrink-0">
            <Plus size={18} /> Notify Candidates
          </button>
