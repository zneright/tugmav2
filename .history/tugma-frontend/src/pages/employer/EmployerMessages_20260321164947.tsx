...
import {
  ...
  FileText, Loader2, MessageSquare, Printer
} from 'lucide-react';
...
export default function EmployerMessages() {
  ...
  const loadConversations = async (currentUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // 🔥 NEW: Check if we navigated here from another page with a specific user ID in the URL
  ...
  // 2. Handle Bridge from Applicant Page (Auto-send first message!)
  useEffect(() => {
  ...
  const handlePrintDTR = async () => {
    ...
    try {
      ...
      let logs: any[] = [];
      let profile = { firstName: activeChat.name.split(' ')[0], lastName: activeChat.name.split(' ')[1] || '', course: 'Not specified' };
      let ojt = { requiredHours: 450, completedHours: 0 };

      if (logsRes.ok) {
        const logsData = await logsRes.json();
        logs = Array.isArray(logsData) ? logsData : [];
        ojt.completedHours = logs.reduce((sum, log) => sum + (Number(log.hoursCredited) || 0), 0);
      }

      if (profRes.ok) {
        const profData = await profRes.json();
        if (profData.firstName) profile.firstName = profData.firstName;
        if (profData.lastName) profile.lastName = profData.lastName;
        if (profData.course) profile.course = profData.course;

        if (profData.ojt) {
          const parsedOjt = typeof profData.ojt === 'string' ? JSON.parse(profData.ojt) : profData.ojt;
          if (parsedOjt.requiredHours) ojt.requiredHours = parsedOjt.requiredHours;
        }
      }

      const printWindow = window.open('', '_blank');
      if (!printWindow) {
        alert("Pop-up blocked! Please allow pop-ups to print the DTR.");
        setIsPrinting(false);
        return;
      }
      ...
      const html = `
        <html>
          <head>
            <title>Official DTR - ${profile.firstName} ${profile.lastName}</title>
            <style>
          ...
          <body>
            <div class="header">
              <h1>Official Daily Time Record (DTR)</h1>
            ...
            <div class="info">
              ...
              <div style="text-align: right;">
                <strong>Target Hours:</strong> ${ojt.requiredHours}<br>
            ...
            <table>
              ...
              <tbody>
                ...
                `).join('')}
                ${logs.length === 0 ? '<tr><td colspan="4" style="text-align:center;">No logs recorded yet.</td></tr>' : ''}
              </tbody>
            </table>
            <div class="summary">
              Total Validated Hours: <span class="highlight">${ojt.completedHours} / ${ojt.requiredHours}</span>
            </div>
            <script>
      ...
      console.error("Failed to print DTR:", error);
