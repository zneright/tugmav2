...
import {
  ...
  MoreVertical, GraduationCap, Clock, Loader2, Download
} from 'lucide-react';
...
export default function ConnectedStudents() {
  ...
  const fetchEmployerStudents = async (employerUid: string) => {
    try {
      ...
      // 2. Map through applications and fetch real DTR hours for "Hired" students
      const processedStudents = await Promise.all(applications.map(async (app: any) => {
        let hoursLogged = 0;
        let totalHours = 450;
        let displayStatus = app.status;

        // If hired, they are an Active Intern -> Fetch their actual progress
        if (app.status === 'Hired') {
          displayStatus = 'Active Intern';
        ...
        const colors = ['bg-purple-600', 'bg-blue-600', 'bg-amber-600', 'bg-indigo-600'];
        ...
        return {
          ...
          color: app.status === 'Hired' ? 'bg-emerald-600' : randomColor, // Active interns are always green
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesStatus = filterStatus === 'All Statuses' ||
      ...
      (filterStatus === 'Active Interns' && student.status === 'Active Intern');
    ...
    <div className="space-y-6 fade-in pb-10">
      ...
      {filteredStudents.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm">
          ...
          <h3 className="text-lg font-bold text-zinc-900 dark:text-white">No interns found</h3>
          <p className="text-sm text-zinc-500 mt-1">Try adjusting your filters or wait for new applications.</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-4 flex-1">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider ${student.status === 'Active Intern' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' :
                      student.status === 'Completed' ? 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400' :
                        student.status === 'Shortlisted' ? 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400' :
