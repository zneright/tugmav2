...
import { formatDistanceToNow } from 'date-fns'; // Highly recommended to install date-fns for nice times: npm install date-fns
...
export default function EmployerNotifications() {
  ...
  const handleSendNotification = async () => {
    ...
    setIsSending(true);
    try {
      // Split the filter to extract job_id and status (e.g. "5_Shortlisted")
      let payload: any = {
        employer_uid: uid,
  ...
  // Filter local notifications by search term
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          ...
          filteredNotifications.map((n) => {
            ...
            try {
              timeString = formatDistanceToNow(new Date(n.created_at), { addSuffix: true });
            } catch (e) { }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] font-medium text-zinc-400 uppercase tracking-wider shrink-0 ml-2">{timeString}</span>
                  </div>
