...
interface Student {
  ...
  logs: any[];    // 🔥 NEW: Store logs for printing
  profile: any;   // 🔥 NEW: Store profile for printing
}
...
export default function ConnectedStudents() {
    ...
    try {
      ...
      const processedStudents = await Promise.all(hiredApplications.map(async (app: any) => {
        ...
        return {
          ...
          logs: fetchedLogs,        // 🔥 Saved for the PDF Modal
          profile: fetchedProfile   // 🔥 Saved for the PDF Modal
        };
      }));
  ...
  const handlePrintClick = (student: Student) => {
    ...
    // 🔥 LOG THE EVENT 🔥
    logSystemEvent('Generated DTR Review', `Opened DTR print preview for intern: ${student.name} (${student.hoursLogged}/${student.totalHours} hrs)`);
  };
