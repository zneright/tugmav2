...
export default function EmployerDashboard() {
  ...
  const fetchDashboardData = async (employerUid: string) => {
    try {
      // 1. Fetch Employer's Applicants
      const appsRes = await fetch(`http://localhost:8080/api/applications/employer/${employerUid}`);
      let appsData: any[] = [];
      if (appsRes.ok) {
        appsData = await appsRes.json();
      }

      // 2. Fetch All Jobs and filter for this specific employer
      const jobsRes = await fetch(`http://localhost:8080/api/jobs/active`);
      let employerJobs: any[] = [];
      if (jobsRes.ok) {
        const allJobs = await jobsRes.json();
        employerJobs = allJobs.filter((j: any) => j.firebase_uid === employerUid);
      }

      // --- PROCESS THE REAL DATA ---

      // Calculate Stats
      const shortlistedCount = appsData.filter(a => a.status === 'Shortlisted').length;
      setDashboardStats({
        activeJobs: employerJobs.length,
        totalApplicants: appsData.length,
        shortlisted: shortlistedCount,
        views: appsData.length * 3 // Mock multiplier for views since we don't have a direct views endpoint for employers yet
      });

      // Format Recent Applicants (Sort by newest, grab top 4)
      ...
      // Format Active Jobs (Grab top 3, count their specific applicants)
      const formattedJobs = employerJobs.slice(0, 3).map(job => {
        const jobAppCount = appsData.filter(a => a.job_id === job.id).length;
        return {
          id: job.id,
          title: job.title,
          applicants: jobAppCount,
          daysLeft: 14, // Mock days left
          status: jobAppCount > 10 ? 'Urgent' : 'Active'
        };
      });
  ...
  // Compile the cards array dynamically based on state
  const statCards = [
