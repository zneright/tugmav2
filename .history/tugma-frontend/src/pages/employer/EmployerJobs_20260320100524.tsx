...
export default function EmployerJobs() {
  ...
  const [companyLocation, setCompanyLocation] = useState<string>('Company HQ'); // 👈 Holds Employer's Address
  ...
  const [formData, setFormData] = useState({
    ...
    work_setup: 'Onsite',
    salary: '',
    locationSelection: '', // Will hold 'hq' or 'other'
    customLocation: '',
    days_per_week: '5',
    hours_per_day: '8',
    description: '',
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUid(user.uid);
        fetchJobs(user.uid);
        fetchEmployerProfile(user.uid); // 👈 Fetch their profile on load
      } else {
        setIsLoading(false);
      }
    });
  ...
  const openCreateModal = () => {
    ...
    setFormData({
      ...
      locationSelection: companyLocation, customLocation: '',
  ...
  const openEditModal = (job: Job) => {
    ...
    // Determine if the job's location is the HQ or a custom one
    ...
    setFormData({
      ...
      customLocation: isHQ ? '' : job.location,
  ...
  const openViewModal = (job: Job) => {
    ...
    setFormData({
      ...
      locationSelection: isHQ ? companyLocation : 'Other', customLocation: isHQ ? '' : job.location,
  ...
  const handleSaveJob = async (status: 'Active' | 'Draft' | 'Closed') => {
    ...
    const finalLocation = formData.locationSelection === 'Other' ? formData.customLocation.trim() : formData.locationSelection;

    if (!formData.title.trim() || !finalLocation || !formData.description.trim() || formData.skills.length === 0) {
      showToast("Please fill in all required fields, including at least one skill!", 'error');
      return;
    }
    ...
    try {
      ...
      submitData.append('location', finalLocation); // Send the correct location
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl relative custom-scrollbar">
            ...
            <div className="sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md z-10 p-6 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">
                  {modalMode === 'view' ? 'Reviewing job details.' : 'All fields are required to attract top student talent.'}
                </p>
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* 👇 NEW: Work Setup Dropdown 👇 */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Work Setup <span className="text-red-500">*</span></label>
                  <select disabled={isReadOnly} name="work_setup" value={formData.work_setup} onChange={handleChange} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-purple-500/20 outline-none transition-all dark:text-white text-sm appearance-none disabled:opacity-70">
                ...
                {/* 👇 NEW: Dynamic Location Logic 👇 */}
                <div className="space-y-1.5 sm:col-span-2 bg-zinc-50 dark:bg-zinc-800/20 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800">
                  <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1 flex items-center gap-1.5"><MapPin size={14} /> Office Location <span className="text-red-500">*</span></label>
                  <div className="flex flex-col sm:flex-row gap-3 mt-2">
                    <select disabled={isReadOnly} name="locationSelection" value={formData.locationSelection} onChange={handleChange} className="w-full sm:w-1/2 px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 outline-none text-sm appearance-none disabled:opacity-70">
                      <option value={companyLocation}>{companyLocation} (Company HQ)</option>
                      <option value="Other">Other Location / Remote</option>
                    </select>
                    {formData.locationSelection === 'Other' && (
                      <input disabled={isReadOnly} name="customLocation" value={formData.customLocation} onChange={handleChange} type="text" placeholder="Type custom location..." className="w-full sm:w-1/2 px-4 py-3 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 outline-none text-sm disabled:opacity-70 animate-in fade-in" />
                    )}
                  </div>
                </div>
              ...
              {/* Description */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Job Description <span className="text-red-500">*</span></label>
              ...
              {/* Skills Tags */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Required Skills <span className="text-red-500">*</span> {!isReadOnly && "(Tap Enter to add skill)"}</label>
          ...
          {filteredJobs.map(job => (
            <div key={job.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow group flex flex-col sm:flex-row gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-3 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                  {/* 👇 NEW: Display Work Setup on Card 👇 */}
                  <span className="flex items-center gap-1"><MonitorSmartphone size={12} /> {job.work_setup}</span>
                  ...
                  <span className="flex items-center gap-1 text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 px-2 py-0.5 rounded-md">
                    <CalendarClock size={12} /> {job.days_per_week}d/wk • {job.hours_per_day}h/d
                  </span>
                ...
                <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => openEditModal(job)} className="p-2 bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-800/50 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-lg transition-colors"><Edit2 size={16} /></button>
                    <button onClick={() => openViewModal(job)} className="p-2 bg-purple-50 hover:bg-purple-100 dark:bg-purple-500/10 dark:hover:bg-purple-500/20 text-purple-600 dark:text-purple-400 rounded-lg transition-colors font-bold text-xs flex items-center gap-1"><Eye size={16} /> View</button>
                  </div>
                </div>
              </div>
            </div>
