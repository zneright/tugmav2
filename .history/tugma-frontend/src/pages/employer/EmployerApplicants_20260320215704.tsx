...
interface Applicant {
  ...
  status: string;
  applied_at: string;
  student_skills: string[]; // Parsed from JSON
  job_skills: string[]; // Parsed from JSON
}
...
export default function EmployerApplicants() {
  ...
  const fetchApplicants = async (employerUid: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Safely parse the JSON strings from the backend into real Arrays
  ...
  const handleStatusChange = async (applicationId: number, newStatus: string) => {
    ...
    // Optimistic UI update in the modal if it's open
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-2xl overflow-y-auto max-h-[90vh] shadow-2xl custom-scrollbar border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="relative">
              <div className={`h-32 w-full ${selectedApplicant.match_percentage >= 80 ? 'bg-emerald-500' : selectedApplicant.match_percentage >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} />
              ...
              <div className="px-6 sm:px-10 pb-6 -mt-12 relative z-10 flex flex-col sm:flex-row gap-5 sm:items-end">
                ...
                <div className="flex-1 bg-white dark:bg-zinc-900 p-4 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 mt-2 sm:mt-0">
                  ...
                  <p className="text-zinc-500 dark:text-zinc-400 font-medium text-sm mt-0.5 truncate">
                    {selectedApplicant.course || "IT Student"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 sm:px-10 pb-8 space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-6">
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => alert("Student Public Profile routing will go here!")} className="text-sm font-bold text-purple-600 dark:text-purple-400 hover:underline flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Match Score */}
                <div className="bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                  <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Match Score</p>
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2.5 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${selectedApplicant.match_percentage >= 80 ? 'bg-emerald-500' : selectedApplicant.match_percentage >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${selectedApplicant.match_percentage}%` }}></div>
                    </div>
                  ...
                  <div className="col-span-1 md:col-span-1 flex items-center justify-end gap-2 mt-4 md:mt-0">
                    <button
                      onClick={() => {
                        ...
                        if (app.status === 'New Applicant') handleStatusChange(app.application_id, 'Reviewed'); // Auto-review when opened
                      }}
