...
export default function EmployerJobs() {
  ...
  const handleCreateJob = async (status: 'Active' | 'Draft') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchJobs(uid);
        // Automatically switch back to "All Jobs" so they can see their new post
  ...
  // 👇 NEW: Filter the jobs based on the active tab 👇
      ...
      {filteredJobs.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl p-10 flex flex-col items-center justify-center text-center h-64">
          ...
          <p className="text-sm text-zinc-500 mt-1 max-w-sm">
            {activeTab === 'All Jobs'
              ? 'Click the "Post New Job" button above to create your first listing.'
              : `You don't have any jobs marked as ${activeTab}.`}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {/* 👇 NEW: Mapping over filteredJobs 👇 */}
