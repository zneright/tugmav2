...
export default function EmployerNotifications() {
  ...
  // 🔥 UPDATED: Added explicitly styling for Admin "alert" type
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'alert': return <ShieldAlert size={18} className="text-red-500" />; // Admin Broadcasts
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
              ...
              <button onClick={() => setRecipientModal({ ...recipientModal, isOpen: false })} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="max-h-[400px] overflow-y-auto p-4 space-y-2 custom-scrollbar">
              {recipientModal.loading ? (
                ...
                recipientModal.data.map((r, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-700">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center text-purple-600 font-bold">
                        {r.first_name[0]}
                      </div>
                      <div>
                    ...
                    {r.is_read == 1 ? (
                      ...
                      <span className="text-[10px] font-bold text-zinc-400 bg-zinc-100 dark:bg-zinc-700 px-2 py-1 rounded-md">SENT</span>
                    )}
                  </div>
                ))
              )}
      ...
      <div className="space-y-3">
        {filteredList.length === 0 ? (
          ...
          filteredList.map((n) => {
            ...
            const isAlert = n.type === 'alert'; // 🔥 Identify Admin Broadcasts

            // 🔥 Dynamic styling for Admin Alerts vs Normal Messages
            ...
            if (viewMode === 'sent') {
              ...
              cardClass = isRead ? 'bg-white dark:bg-zinc-900 border-red-100 dark:border-red-900/30 opacity-90' : 'bg-red-50/30 dark:bg-red-500/5 border-red-200 dark:border-red-500/30 shadow-md shadow-red-500/10 hover:border-red-400';
            } else {
            ...
            return (
              <div
                ...
                className={`group p-5 rounded-3xl border flex gap-4 items-start transition-all cursor-pointer ${cardClass}`}
              >

                {viewMode === 'inbox' && (
                  <div className={`p-3 rounded-2xl shrink-0 mt-1 transition-colors ${isAlert
                    ? (isRead ? 'bg-red-50 dark:bg-red-900/10' : 'bg-red-100 dark:bg-red-500/20')
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <h3 className={`text-[15px] font-bold ${isAlert && !isRead && viewMode === 'inbox'
                      ...
                      {/* 🔥 Label for Admin Alerts */}
                      {isAlert && viewMode === 'inbox' && (
                        <span className="text-[10px] font-black uppercase tracking-wider bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400 px-2 py-0.5 rounded-md border border-red-200 dark:border-red-900/50">
                          System Alert
                        </span>
                      )}
                    </div>
                    <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">{formatDate(n.created_at)}</span>
                  </div>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed pr-8">{n.message}</p>
                </div>

                {viewMode === 'inbox' ? (
                  <button onClick={() => handleDeleteInbox(n.id)} className="p-2 text-zinc-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors opacity-0 group-hover:opacity-100">
                    <Trash2 size={16} />
                  </button>
                ) : (
                  <button onClick={(e) => handleUnsendBlast(e, n)} className="p-2 text-zinc-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors opacity-0 group-hover:opacity-100" title="Delete Blast">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
