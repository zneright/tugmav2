...
export default function EmployerMessages() {
  ...
  // Handle incoming chats from the Applicant Tracking page
  useEffect(() => {
    const pendingChat = localStorage.getItem('pending_new_chat');
    if (pendingChat) {
      try {
        const chatData = JSON.parse(pendingChat);
        const existingChat = conversations.find(c => c.name === chatData.name);
        ...
        if (existingChat) {
          ...
          const newConversation = {
            id: chatData.id,
            name: chatData.name,
            role: chatData.role || 'Applicant',
            avatar: chatData.avatar,
            lastMessage: 'New connection started',
            time: 'Just now',
            unread: 0,
            online: true,
            color: 'bg-purple-600'
          ...
          setMessages([{
            id: Date.now(),
            sender: 'me',
            text: `Hi ${chatData.name.split(' ')[0]}, thanks for applying to the ${chatData.role || 'open'} position! We are reviewing your application now.`,
