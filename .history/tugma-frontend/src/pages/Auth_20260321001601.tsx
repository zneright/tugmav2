...
import { Mail, Briefcase, ShieldAlert, Lock, X, AlertCircle, User, Building, Users } from 'lucide-react';
...
import {
  ...
  signInWithPopup,
  signOut // Added signOut to clean up cancelled Google logins
} from 'firebase/auth';
...
export default function Auth() {
  ...
  // NEW: Google Role Selection Modal
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin || authView === 'admin') {
        ...
        if (!response.ok) {
          // If MySQL fails (like complaining about profile_data), we DELETE the Firebase user so they aren't stuck in limbo
          await user.delete();
      ...
      else setError(`Server Error: ${err.message.replace('Firebase: ', '')}`); // Expose the exact backend error!
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSocialLogin = async (provider: any) => {
    ...
    try {
      ...
      setLoading(false); // Stop loading so they can see the modal
    }
  };
  ...
  const finalizeGoogleSignUp = async (selectedRole: SignUpRole) => {
    ...
    try {
      ...
      console.error(err);
      setError(`Google Sync Error: ${err.message}`); // Expose the exact backend error!
    } finally {
      setLoading(false);
          ...
          <div className="bg-[#130924] border border-[#913ceb]/50 w-full max-w-sm rounded-3xl p-8 shadow-[0_0_40px_rgba(145,60,235,0.15)] relative flex flex-col items-center text-center">
            ...
            <button
              onClick={() => {
                setShowRoleModal(false);
                setPendingGoogleUser(null);
                signOut(auth); // Clean up Firebase if they cancel
              }}
