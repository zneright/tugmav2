...
import tugmaLogo from '../assets/tugma_logo3.png';
...
export default function Auth() {
  ...
  // Handle standard email/password login and registration
    ...
    try {
      if (isLogin || authView === 'admin') {
        ...
        if (response.ok) {
          ...
          const trueRole = data.role?.toLowerCase();
          const trueStatus = data.status?.toLowerCase() || 'active';

          const isAdminRole = trueRole === 'admin' || trueRole === 'superadmin';

          if (trueStatus === 'archived') {
            await signOut(auth);
            setError('This account has been archived. Please contact support!');
            setLoading(false);
            return;
          }
          if (trueStatus === 'banned') {
            await signOut(auth);
          ...
          if (authView !== 'admin' && isAdminRole) {
            await signOut(auth);
            setError('Admin accounts must log in through the Admin Access portal.');
            setLoading(false);
            return;
          }

          logSystemEvent(firebaseUid, 'User Login', `Logged in via Email/Password as ${trueRole.toUpperCase()}`);
          navigate(isAdminRole ? '/admin/dashboard' : trueRole === 'student' ? '/dashboard' : '/employer/dashboard');
        } else {
          await signOut(auth);
  ...
  // Synchronize Google Registration to DB
  ...
  // Dispatch password reset email via Firebase
    ...
    <div className="h-screen w-full flex font-sans bg-[#0a0310] overflow-hidden">
      ...
      <div className="hidden lg:flex w-1/2 h-full relative bg-cover bg-right" style={{ backgroundImage: `url('${bgImage}')` }}>
        <div className="absolute top-10 left-12 z-20">
          {/* Beautiful Circle Border Logo */}
          <div className="w-36 h-36 xl:w-44 xl:h-44 bg-white rounded-full flex items-center justify-center border-[4px] border-[#913ceb] shadow-[0_0_40px_rgba(145,60,235,0.5)] overflow-hidden p-3 transition-transform hover:scale-105">
            <img src={tugmaLogo} alt="Tugma Logo" className="w-full h-full object-contain" />
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col overflow-y-auto custom-scrollbar bg-[#0a0310]">
        ...
        <div className="lg:hidden w-full flex justify-center pt-10 pb-2 shrink-0">
          {/* Beautiful Circle Border Logo for Mobile */}
          <div className="w-28 h-28 bg-white rounded-full flex items-center justify-center border-4 border-[#913ceb] shadow-[0_0_30px_rgba(145,60,235,0.4)] overflow-hidden p-2 transition-transform hover:scale-105">
            <img src={tugmaLogo} alt="Tugma Logo" className="w-full h-full object-contain" />
          </div>
        </div>
