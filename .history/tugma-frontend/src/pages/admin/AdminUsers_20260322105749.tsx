...
class Users extends ResourceController {
  ...
  // 🔥 NEW: BACKEND SILENT AUDIT LOGGER 🔥
  private function logSystemEvent($uid, $action, $details) {
    try {
      ...
      if ($user) {
        ...
        // If user is admin, check if they are the superadmin
        if (strtolower($user -> role) === 'admin') {
          if ($user -> email === self:: SUPERADMIN_EMAIL || $user -> email === self::SUPERADMIN_EMAIL_ALT) {
            $role = 'Super Admin';
          }
        }
      }

      // Append role and name to details for absolute clarity
      $fullDetails = "[{$role}] {$userName} -> {$details}";
      ...
      // Silently fail so it never interrupts the main user flow
  ...
  public function register() {
    try {
      ...
      $this -> logSystemEvent($data['firebase_uid'], 'Registered Account', "Created a new {$roleLabel} account.");
  ...
  public function updateProfile($uid = null) {
    try {
      ...
      // 🔥 LOG THE EVENT 🔥
      $this -> logSystemEvent($uid, 'Updated Profile', 'User updated their personal profile details.');
