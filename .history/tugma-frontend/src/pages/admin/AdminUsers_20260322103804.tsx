...
import {
  ...
  Ban, Mail, Shield, Loader2, Download, Trash2, Unlock, Plus, X
} from 'lucide-react';
import { auth } from '../../firebaseConfig';
import { createUserWithEmailAndPassword } from 'firebase/auth';

// 🔥 SUPERADMIN EMAIL (Protected from Deletion)
...
export default function AdminUsers() {
  ...
  useEffect(() => {
    // Get the currently logged-in user so we know who is viewing this page
    if (auth.currentUser) {
  ...
  const exportToCSV = () => {
    ...
    // 🔥 LOG THE SECURITY EVENT 🔥
    logSystemEvent('Exported User Database', `Exported full database of ${users.length} users to CSV.`);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {currentUserEmail === SUPERADMIN_EMAIL && (
            <button onClick={() => setIsModalOpen(true)} className="flex items-center justify-center gap-2 bg-red-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md shadow-red-500/20 transition-all hover:bg-red-700 shrink-0">
              <Plus size={16} /> New Admin
            </button>
          )}
          <button onClick={exportToCSV} className="flex items-center justify-center gap-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition-all hover:opacity-90 shrink-0">
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((user) => {
                // 🔥 THE PROTECTION LOGIC: Is this row the Superadmin?
                  ...
                  <tr key={user.id} className={`hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group ${isSuperAdmin ? 'bg-red-50/30 dark:bg-red-500/5' : ''}`}>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {/* Hide Delete/Ban if it's the Superadmin */}
                        {!isSuperAdmin && (
                          <>
                            <button
