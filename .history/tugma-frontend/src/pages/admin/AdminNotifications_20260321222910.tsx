...
import { Bell, Megaphone, X, Send, ShieldAlert, Loader2, Trash2, Users } from 'lucide-react';
...
export default function AdminNotification() {
  ...
  const fetchBroadcastHistory = async (adminUid: string) => {
    try {
      // Reusing the same grouped "Sent" endpoint we built for employers!
  ...
  const handleUnsendBroadcast = async (b: Broadcast) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6 fade-in relative pb-10">
      ...
      <div className="space-y-3">
        {broadcasts.length === 0 ? (
          ...
          broadcasts.map((b) => (
            <div key={b.id} className="group p-5 rounded-3xl border bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:border-red-300 dark:hover:border-red-900/50 flex gap-4 items-start transition-all shadow-sm">
              ...
              <div className="flex-1 min-w-0 pt-0.5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1 gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-[15px] font-bold text-zinc-900 dark:text-white leading-tight">{b.title}</h3>
                    <span className="text-[10px] font-bold bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400 px-2 py-0.5 rounded-md flex items-center gap-1 border border-red-200 dark:border-red-500/30 whitespace-nowrap">
                      <Users size={10} /> Reached {b.recipient_count}
                    </span>
                  </div>
              ...
              <div className="flex flex-col items-end shrink-0">
                <button
                  onClick={() => handleUnsendBroadcast(b)}
