  ...
  export default function AdminUsers() {
    ...
    // Admin Creation Modal States (🔥 Updated to use firstName and lastName)
    ...
    const handleCreateAdmin = async (e: React.FormEvent) => {
      e.preventDefault();
      // 🔥 Check for both firstName and lastName
      ...
      try {
        ...
        const res = await fetch('http://localhost:8080/api/users/register', {
          ...
          body: JSON.stringify({
            ...
            role: 'admin',
            firstName: newAdmin.firstName, // 🔥 Direct mapping
            lastName: newAdmin.lastName    // 🔥 Direct mapping
          })
        });
    ...
    const handleArchiveUser = async (uid: string, currentStatus: string) => {
      ...
      try {
        // Use the same status endpoint to archive the user
        const res = await fetch(`http://localhost:8080/api/admin/users/${uid}/status`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ status: newStatus })
        ...
        logSystemEvent(`${isArchived ? 'Restored' : 'Archived'} User`, `Changed status to ${newStatus} for user: ${targetUser?.email || uid}`);

      } catch (error) {
        alert(`Network Error: Could not ${actionText.toLowerCase()} user.`);
        // Revert on fail
        setUsers(prev => prev.map(u => u.id === uid ? { ...u, status: currentStatus } : u));
      }
    };

    // 🔥 UPDATED FILTER LOGIC (Hides archived users from main tabs)
    const filteredUsers = users.filter(user => {
      const isUserArchived = user.status.toLowerCase() === 'archived';

      let matchesTab = false;
      if (activeTab === 'All') matchesTab = !isUserArchived; // Hide archived from 'All'
      else if (activeTab === 'Archived') matchesTab = isUserArchived;
      else if (activeTab === 'Banned') matchesTab = user.status.toLowerCase() === 'banned';
      else if (activeTab === 'Students') matchesTab = user.role.toLowerCase() === 'student' && !isUserArchived;
      else if (activeTab === 'Employers') matchesTab = user.role.toLowerCase() === 'employer' && !isUserArchived;
      else if (activeTab === 'Admins') matchesTab = user.role.toLowerCase() === 'admin' && !isUserArchived;
    ...
    const getStatusBadge = (status: string) => {
      switch (status.toLowerCase()) {
        ...
        case 'archived': return <span className="flex items-center gap-1 w-fit px-2 py-1 bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 rounded-lg text-[10px] font-bold uppercase tracking-wider"><Archive size={12} /> Archived</span>;
      ...
      <div className="space-y-6 fade-in pb-10 relative">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-md rounded-3xl p-8 shadow-2xl relative">
              ...
              <form onSubmit={handleCreateAdmin} className="space-y-4">
                {/* 🔥 SPLIT FIRST NAME AND LAST NAME SIDE-BY-SIDE */}
                <div className="flex gap-4">
                  <div className="flex-1">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">First Name</label>
                    <input required type="text" value={newAdmin.firstName} onChange={(e) => setNewAdmin({ ...newAdmin, firstName: e.target.value })} className="w-full px-4 py-3 mt-1 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-red-500/20 outline-none text-sm dark:text-white" />
                  </div>
                  <div className="flex-1">
                    <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider ml-1">Last Name</label>
                    <input required type="text" value={newAdmin.lastName} onChange={(e) => setNewAdmin({ ...newAdmin, lastName: e.target.value })} className="w-full px-4 py-3 mt-1 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-red-500/20 outline-none text-sm dark:text-white" />
                  </div>
                </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl w-fit overflow-x-auto custom-scrollbar">
            {/* 🔥 ADDED ARCHIVED TAB HERE */}
            {['All', 'Students', 'Employers', 'Admins', 'Banned', 'Archived'].map((tab) => (
                    ...
                    <tr key={user.id} className={`hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group ${isSuperAdminRow ? 'bg-red-50/30 dark:bg-red-500/5' : ''}`}>
                      ...
                      <td className="p-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {!isSuperAdminRow && (
                            <>
                              {/* Ban / Unban Button (Disabled visually if archived to keep logic clean, or keep it active) */}
                              <button
                                ...
                                className={`p-2 rounded-lg transition-colors ${user.status.toLowerCase() === 'banned'
                                  ...
                                  } ${user.status.toLowerCase() === 'archived' ? 'opacity-30 cursor-not-allowed' : ''}`}
                                title={user.status.toLowerCase() === 'banned' ? 'Unban User' : 'Ban User'}
                                disabled={user.status.toLowerCase() === 'archived'}
                              >
                              ...
                              <button
                                onClick={() => handleArchiveUser(user.id, user.status)}
                                className={`p-2 rounded-lg transition-colors ${user.status.toLowerCase() === 'archived'
                                  ? 'text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10' // Restore styling
                                  : 'text-zinc-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10' // Archive styling
                                  }`}
                                title={user.status.toLowerCase() === 'archived' ? 'Restore User' : 'Archive User'}
                              >
                                {user.status.toLowerCase() === 'archived' ? <ArchiveRestore size={16} /> : <Archive size={16} />}
                              </button>
                            </>
                          )}
                        </div>
