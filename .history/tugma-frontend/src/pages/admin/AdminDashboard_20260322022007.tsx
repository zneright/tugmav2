...
export default function AdminDashboard() {
  ...
  const fetchAllAdminStats = async () => {
    ...
    try {
      ...
      if (statsResponse.ok && logsResponse.ok) {
        ...
        setData(statsResult);
        setAuditLogs(logsResult.slice(0, 8)); // Grab the 8 most recent movements
      }
    } catch (error) {
  ...
  // Logic to format time from audit logs
  ...
  const getActivityIcon = (role: string) => {
    if (role === 'Employer') return <Building2 size={14} className="text-purple-500" />;
    if (role === 'Admin' || role === 'System') return <Server size={14} className="text-red-500" />;
