...
export default function AdminNotification() {
  ...
  const handleSendBroadcast = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logSystemEvent('Sent Admin Broadcast', `Target: ${broadcastData.target}. Title: "${broadcastData.title}"`);

        alert(`Success! Broadcast sent to ${result.count} users.`);
        setIsModalOpen(false);
        setBroadcastData({ title: '', message: '', target: 'ALL' });
        fetchBroadcastHistory(uid);
      } else {
  ...
  const handleUnsendBroadcast = async (e: React.MouseEvent, b: Broadcast) => {
    ...
    try {
      ...
      logSystemEvent('Unsent Admin Broadcast', `Retracted broadcast: "${b.title}"`);
