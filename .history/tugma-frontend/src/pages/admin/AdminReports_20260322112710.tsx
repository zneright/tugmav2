...
import ExportPdfModal from '../../components/ExportPdfModal'; // Adjust import path
...
export default function AdminReports() {
  ...
  const [adminRole, setAdminRole] = useState(''); // 🔥 Added role state
  ...
  useEffect(() => {
    ...
    // Authenticate and grab the current admin's details
      ...
      if (user) {
        ...
        try {
          ...
          if (res.ok) {
            ...
            // 🔥 Save the real role from the database
            if (data.role) setAdminRole(data.role.toLowerCase());
          }
        } catch (error) {
  ...
  const filteredLogs = logs.filter(log => {
    ...
    // 🔥 Made case-insensitive so it matches the DB accurately
  ...
  const exportToCSV = () => {
    ...
    logSystemEvent('Exported Audit Logs', `Exported ${filteredLogs.length} logs to CSV with role filter: ${filterRole}`);
  };

  // 🔥 Dynamic Superadmin Check
  const isSuperAdmin = adminRole === 'superadmin';

  let filterTextDesc = `Active Filters: Role -> ${filterRole}`;
  if (searchQuery) filterTextDesc += ` | Search -> "${searchQuery}"`;
  if (startDate || endDate) filterTextDesc += ` | Dates -> ${startDate || 'Any'} to ${endDate || 'Any'}`;

  // Formatted Rows for PDF
  const pdfRows = filteredLogs.map(log => [
    log.id,
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 fade-in pb-10">
      ...
      <ExportPdfModal
        ...
        columns={["Log ID", "Time", "User", "Role", "Action", "Details"]}
        rows={pdfRows}
        reportTitle="SYSTEM AUDIT TRAIL"
        fileName={`tugma_audit_log_${new Date().toISOString().split('T')[0]}.pdf`}
        adminName={adminName}
        isSuperAdmin={isSuperAdmin}
        filterText={filterTextDesc}
        onSuccess={() => logSystemEvent('Generated Audit Report', `Exported ${filteredLogs.length} logs to PDF with role filter: ${filterRole}`)}
      />
