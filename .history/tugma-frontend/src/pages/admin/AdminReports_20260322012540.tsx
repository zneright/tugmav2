...
import {
  ...
  Search, ShieldAlert, Sparkles, Loader2, RefreshCw, FileText
} from 'lucide-react';
...
const SUPERADMIN_EMAIL_ALT = "buday.3132578@caloocan.sti.edu.ph"; // Just in case of typo!
...
export default function AdminReports() {
  ...
  // Advanced Filter Logic (Includes Time Range)
  ...
  // 🔥 NEW: High-Quality PDF Export with Dark Header, Logo, and Admin Info
  const exportToPDF = async () => {
    ...
    // 1. Create Image Object to load the local React asset
    const img = new Image();
    img.src = tugmaLogo;

    // Wait for the image to load so jsPDF doesn't render it blank
    await new Promise((resolve) => {
      img.onload = resolve;
      img.onerror = resolve; // Resolve anyway to prevent freezing if image path breaks
    });
    ...
    // --- HEADER BACKGROUND ---
    doc.setFillColor(15, 9, 30); // Very dark purple to make the white logo pop
    doc.rect(0, 0, pageWidth, 40, 'F');

    // --- ADD LOGO ---
    try {
      doc.addImage(img, 'PNG', 14, 8, 28, 24);
    } catch (error) {
      console.warn("Logo failed to load into PDF", error);
    }
    ...
    doc.text("SYSTEM AUDIT TRAIL", 48, 20);
    ...
    doc.text(`Generated by: ${adminName} (${roleTag})`, 48, 28);
    ...
    if (startDate || endDate) filterText += ` | Date Range -> ${startDate || 'Any'} to ${endDate || 'Any'}`;
    ...
    autoTable(doc, {
      head: [tableColumn],
      body: tableRows,
      startY: 55, // Push table down below the dark header and filters text
      styles: { fontSize: 8, cellPadding: 3 },
      headStyles: { fillColor: [220, 38, 38], textColor: 255, fontStyle: 'bold' }, // Red theme for Admin
    ...
    <div className="max-w-6xl mx-auto space-y-6 fade-in pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={exportToPDF} className="bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-red-500/20 transition-all flex items-center gap-2 active:scale-95">
