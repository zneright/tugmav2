...
import {
  ...
  CheckCircle2, Clock, AlertCircle, ChevronDown, Mail, Loader2, Send, Bell
} from 'lucide-react';
...
export default function CustomerService() {
  ...
  // --- NEW: Handle Sending In-App Notification Reply ---
  const handleSendReply = async (ticket: Ticket) => {
    if (!replyMessage.trim()) return;
    setIsSendingReply(true);

    try {
      const response = await fetch(`http://localhost:8080/api/admin/tickets/${ticket.db_id}/reply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: replyMessage, ticket_id: ticket.id, subject: ticket.subject })
      });

      if (response.ok) {
        alert("Reply successfully sent to the user's notifications!");
        setReplyingTo(null);
        setReplyMessage('');
      } else {
          ...
          filteredTickets.map((ticket) => {
            ...
            return (
              <div
                key={ticket.id}
                onClick={() => {
                  setExpandedTicket(isExpanded ? null : ticket.db_id);
                  if (isExpanded) setReplyingTo(null); // Close reply box if collapsing
                }}
              ...
              >
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[800px] mt-4 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        {/* 1. Reply via Email (Original) */}
                        <a
                          href={`mailto:${ticket.email}?subject=Re: ${ticket.subject} [${ticket.id}]`}
                          onClick={(e) => e.stopPropagation()}
                          className="flex-1 sm:flex-none px-3 py-2.5 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
                          title="Reply via Email Client"
                        >
                          <Mail size={14} /> <span className="hidden sm:inline">Email</span>
                        </a>

                        {/* 2. In-App Notification Reply (NEW) */}
                        <button
                          onClick={(e) => { e.stopPropagation(); setReplyingTo(ticket.db_id); }}
                          className="flex-1 sm:flex-none px-3 py-2.5 bg-purple-50 hover:bg-purple-100 dark:bg-purple-500/10 dark:hover:bg-purple-500/20 text-purple-600 dark:text-purple-400 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5 border border-purple-200 dark:border-purple-900/50"
                        >
                          <Bell size={14} /> In-App Reply
                        </button>
                    ...
                    {/* --- NEW: In-App Reply Form Area --- */}
                    {replyingTo === ticket.db_id && (
                      <div
                        onClick={(e) => e.stopPropagation()}
                        className="mt-4 p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700 animate-in slide-in-from-top-2 duration-200"
                      >
                        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                          <Send size={12} /> Draft Notification Response
                        </h4>
                        <textarea
                          autoFocus
                          value={replyMessage}
                          onChange={(e) => setReplyMessage(e.target.value)}
                          placeholder={`Type your reply to ${ticket.user}...`}
                          className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-3 text-sm text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-red-500/20 resize-none"
                          rows={3}
                        />
                        <div className="flex justify-end gap-2 mt-3">
                          ...
                          <button
                            ...
                            className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-red-600/20 transition-all disabled:opacity-50"
                          >
