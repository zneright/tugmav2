  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        // Check Super Admin logic
        const superCheck = user.email === SUPERADMIN_EMAIL || user.email === SUPERADMIN_EMAIL_ALT;
        setIsSuperAdmin(superCheck);
        setUserRole(superCheck ? 'Super Admin' : 'Admin');
      ...
      `}>
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] bg-red-600 text-white px-1.5 py-0.5 rounded uppercase tracking-wider font-bold mt-0.5">Admin</span>
          </div>
          <button
