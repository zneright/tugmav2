...
import { X, FileText, Calendar, PenTool, Settings2, LayoutTemplate, Image as ImageIcon, Clock } from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import tugmaLogo from '../components'
interface LogEntry {
    id?: string;
    date: string;
    hoursLogged: number;
...
export default function PrintDTRModal({ isOpen, onClose, logs, studentProfile, requiredHours, completedHours }: PrintDTRModalProps) {
    ...
    const handleGeneratePDF = () => {
        // Initialize PDF with dynamic paper size (letter = short, legal = long)
        ...
        const drawHeader = (doc: jsPDF, pageNumber: number) => {
            let startX = 105; // Center point for A4/Letter roughly

            if (includeLogo) {
                // Placeholder for Logo - Replace with doc.addImage(base64Logo, 'PNG', 14, 10, 20, 20)
                doc.setDrawColor(200, 200, 200);
                doc.rect(14, 10, 18, 18);
                doc.setFontSize(8);
                doc.setTextColor(150);
                doc.text("LOGO", 23, 20, { align: "center" });
            }
            ...
            if (timeframe === 'specific_month' && specificMonth) periodText += ` (${specificMonth})`;
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            {/* Increased max-width and added flex-col for scrollable content */}
            <div className="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-5 overflow-y-auto space-y-6 flex-1 custom-scrollbar">
                    ...
                    <div className="space-y-3">
                        ...
                        {/* Conditional Inputs based on timeframe */}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {[
                                { id: 'a4', label: 'A4' },
                                { id: 'letter', label: 'Short (Letter)' },
                                { id: 'legal', label: 'Long (Legal)' }
                            ].map((option) => (
                                <button
                                    key={option.id}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <label className="flex items-center gap-3 p-3 border border-zinc-200 dark:border-zinc-700 rounded-xl cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                                ...
                                <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300 flex items-center gap-2">
                                    <ImageIcon size={14} /> Include Logo
                                </span>
                            </label>
