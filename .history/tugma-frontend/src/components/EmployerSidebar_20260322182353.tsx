import { LayoutDashboard, Briefcase, Users, Building2, X, Bell, ChevronDown, LogOut, HelpCircle } from 'lucide-react';
import { NavLink, useNavigate } from 'react-router-dom'; // <-- Removed Link, added useNavigate
...
export default function EmployerSidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const navItems = [
    ...
    { to: '/employer/messages', icon: <Bell size={20} />, label: 'Messages' },
  ...
  return (
    <>
      ...
      `}>
        <div className="h-16 flex items-center justify-center px-4 border-b border-zinc-200 dark:border-zinc-800/50 shrink-0 relative transition-colors">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-1">
              TUGMA <span className="text-[10px] bg-blue-600 text-white px-1.5 py-0.5 rounded uppercase tracking-wider font-bold">Employer</span>
            </span>
          </div>

          <button className="md:hidden absolute right-4 p-1 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors rounded-md" onClick={() => setIsOpen(false)}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800/50 min-w-[16rem] shrink-0 flex flex-col gap-3">
