import { Menu, Bell, User, LogOut, Check, MessageSquare, Briefcase, Zap, UserCheck, ShieldAlert, Shield } from 'lucide-react';
import { useState, useRef, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom'; // <-- Removed Link, added useNavigate
...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  // --- 🔥 NEW ASYNC LOGOUT FUNCTION 🔥 ---
  const handleLogout = async () => {
    try {
      setIsProfileOpen(false); // Close the dropdown
      await signOut(auth); // Wait for Firebase to clear session
    ...
    try {
      ...
      if (isEmployer) {
        ...
        if (res.ok) {
          ...
          if (data.profilePhoto) setAvatarImage(data.profilePhoto);
          else if (data.profile_photo) setAvatarImage(data.profile_photo);
        }
      } else {
        ...
        if (res.ok) {
          ...
          if (fName) {
            setAvatarInitial(fName.charAt(0).toUpperCase());
            setUserName(`${fName} ${lName}`.trim());
          }
  ...
  const handleNotificationClick = async (n: Notification) => {
    ...
    setIsNotificationsOpen(false);
    // Add logic here to navigate to notification detail if desired
  };
        ...
        <div className="flex flex-col justify-center">
          {isAdmin ? (
            userEmail === SUPERADMIN_EMAIL ? (
              <>
                <h1 className="text-base sm:text-lg font-black text-red-600 flex items-center gap-1.5 uppercase tracking-wide">
                  <ShieldAlert size={16} /> SUPER ADMIN PORTAL
                </h1>
              </>
            ) : (
              <>
                <h1 className="text-base sm:text-lg font-black text-zinc-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wide">
                  <Shield size={16} className="text-red-500" /> ADMIN PORTAL
                </h1>
              </>
            )
          ) : (
      ...
      <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="relative" ref={notifyRef}>
            ...
            `}>
              ...
              <div className="p-3 text-center border-t border-zinc-100 dark:border-zinc-800">
                <button onClick={() => { setIsNotificationsOpen(false); navigate(notificationsLink); }} className="text-xs font-bold text-zinc-500 hover:text-purple-600 transition-colors block w-full py-1">
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-sm hover:opacity-90 transition-all border border-zinc-200 dark:border-zinc-700 overflow-hidden shrink-0 ${!avatarImage ? avatarColor : 'bg-zinc-100 dark:bg-zinc-800'}`}
          >
            {avatarImage ? (
              <img
                src={avatarImage}
          ...
          `}>
            <div className="p-1.5 flex flex-col">
              ...
              {!isAdmin && (
                <>
                  <button onClick={() => { setIsProfileOpen(false); navigate(profileLink); }} className="flex items-center gap-2.5 px-3 py-2 w-full text-left text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors font-medium">
              ...
              <button
                ...
                className="flex items-center gap-2.5 px-3 py-2 w-full text-left text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors font-medium"
              >
