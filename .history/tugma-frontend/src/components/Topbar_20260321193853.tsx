...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  // Fetch User Profile for Real Initials/Photo
    ...
    try {
      ...
      if (isEmployer) {
        ...
        if (res.ok) {
          ...
          // If employer has a logo
          if (data.profile_photo) setAvatarImage(data.profile_photo);
        }
      } else {
        ...
        if (res.ok) {
          ...
          if (data.profilePhoto) {
            setAvatarImage(data.profilePhoto);
          } else if (data.profile_photo) { // Catch both casings just in case
  ...
  return (
    <header className="h-16 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-30 px-4 sm:px-6 flex items-center justify-between transition-colors duration-300">
      ...
      {/* Left Section (🔥 ADDED TUGMA LOGO 🔥) */}
      <div className="flex items-center gap-4">
        <button onClick={toggleSidebar} className="p-2 -ml-2 text-zinc-500 hover:text-purple-600 dark:hover:text-purple-400 rounded-md transition-colors lg:hidden">
          <Menu size={24} />
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Logo Text */}
          <h1 className="text-xl font-black bg-gradient-to-r from-purple-600 to-blue-500 bg-clip-text text-transparent hidden sm:block tracking-tight">
            TUGMA
          </h1>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            // 🔥 FIXED: Made button slightly larger (w-9 h-9), ensured overflow-hidden bounds the image
            className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-sm hover:opacity-90 transition-all border-2 border-transparent focus:border-purple-500/50 overflow-hidden shrink-0 ${!avatarImage ? avatarColor : 'bg-zinc-100 dark:bg-zinc-800'}`}
          >
            {/* 🔥 FIXED: Added object-cover to make the Base64 image fit perfectly in the circle */}
            {avatarImage ? (
              <img src={avatarImage} alt="Profile" className="w-full h-full object-cover object-center" />
            ) : (
              avatarInitial
            )}
          </button>
