...
export default function PrintDTRModal({ isOpen, onClose, logs, studentProfile, requiredHours, completedHours }: PrintDTRModalProps) {
    ...
    const handleGeneratePDF = () => {
        ...
        // If "Every Page", we need a larger bottom margin to make room for the signature block
        ...
        autoTable(doc, {
            ...
            didParseCell: function (data) {
                if (data.section === 'body' && filteredLogs[data.row.index].isDouble) {
                    data.cell.styles.fillColor = [250, 245, 255]; // Light purple highlight for double pay
                }
            },
            didDrawPage: function (data) {
                drawHeader(doc, data.pageNumber);

                // Draw signature at the bottom of EVERY page if selected
