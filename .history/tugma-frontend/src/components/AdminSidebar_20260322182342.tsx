import { LayoutDashboard, Users, FileWarning, LifeBuoy, X, LogOut, ShieldAlert, Shield, Bell } from 'lucide-react';
      ...
      `}>
        <div className="h-16 flex items-center justify-center px-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0 relative">
          <div className="flex items-center gap-3">
            <img src={tugmaLogo2} alt="Tugma Logo" className="h-8 w-auto object-contain drop-shadow-sm" />
            <span className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-3.5">
              TUGMA
            </span>
          </div>
          <button className="md:hidden absolute right-4 p-1 text-zinc-400 hover:text-zinc-900 dark:hover:text-white rounded-md transition-colors" onClick={() => setIsOpen(false)}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 shrink-0 flex flex-col gap-3">
          <div className="flex items-center gap-3 p-2 bg-zinc-50 dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-zinc-500 dark:text-zinc-500 truncate uppercase tracking-wider font-bold">{userRole}</p>
            </div>
            {/* 🔥 UPDATED BUTTON 🔥 */}
            <button
