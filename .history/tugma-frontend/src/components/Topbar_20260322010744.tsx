...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  const [userName, setUserName] = useState<string>('User'); // For the Greeting
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUid(user.uid);
        setUserEmail(user.email); // 🔥 Get the email for Superadmin check!
  ...
  // FETCH USER PROFILE FOR PICTURE, INITIALS, AND NAME
  ...
  return (
    <header className="h-16 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-30 px-4 sm:px-6 flex items-center justify-between transition-colors duration-300">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* 🔥 THE SUPERADMIN / ADMIN GREETING LOGIC 🔥 */}
        <div className="hidden sm:block">
          {isAdmin ? (
            userEmail === SUPERADMIN_EMAIL ? (
              <>
                <h1 className="text-lg font-black text-red-600 flex items-center gap-2 leading-none">
                  <ShieldAlert size={18} /> Super Admin Portal
                </h1>
                <p className="text-[11px] uppercase tracking-wider text-zinc-500 dark:text-zinc-400 font-bold mt-1">
                  Welcome back, {userName}
                </p>
              </>
            ) : (
              <>
                <h1 className="text-lg font-black text-zinc-900 dark:text-white flex items-center gap-2 leading-none">
        ...
        <div className="relative" ref={profileRef}>
          ...
          `}>
            <div className="p-1.5 flex flex-col">
              ...
              {/* 🔥 Hide Profile link if they are an Admin 🔥 */}
              {!isAdmin && (
                <>
