...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  // 1. Authenticate and Fetch Notifications
  useEffect(() => {
    ...
    <header className="h-16 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-30 px-4 sm:px-6 flex items-center justify-between transition-colors duration-300">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="relative" ref={notifyRef}>
          ...
          >
            <Bell size={20} />
            {/* 🔥 Real Pulse Dot: Only shows if unread messages exist */}
            {unreadCount > 0 && (
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-sm hover:opacity-90 transition-all border-2 border-transparent focus:border-purple-500/50 ${location.pathname.startsWith('/admin') ? 'bg-red-600' : 'bg-purple-600'
              }`}
          >
            {avatarInitial}
          </button>
