import { LayoutDashboard, Users, FileWarning, LifeBuoy, X, LogOut, ShieldAlert, Shield, Bell } from 'lucide-react'; // 🔥 Added Bell icon
...
export default function AdminSidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  // 🔥 ADDED THE NOTIFICATION ROUTE HERE 🔥
  const navItems = [
    ...
    { to: '/admin/notifications', icon: <Bell size={20} />, label: 'System Alerts' }, // <-- New Link!
  ];
      ...
      `}>
        ...
        <div className="h-16 flex items-center justify-center px-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0 relative">
          <div className="flex items-center gap-3">
            {/* 🔥 Made the logo and text bold and centered properly 🔥 */}
            <img src={tugmaLogo2} alt="Tugma Logo" className="h-10 w-auto object-contain drop-shadow-sm" />
            <span className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-1.5">
              TUGMA <span className="text-[11px] bg-red-600 text-white px-1.5 py-0.5 rounded uppercase tracking-wider font-bold mt-1">Admin</span>
            </span>
          </div>
