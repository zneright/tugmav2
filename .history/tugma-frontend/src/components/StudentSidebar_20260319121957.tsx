import { LayoutDashboard, Briefcase, MessageSquare, User, Bell, X, ChevronDown, HelpCircle } from 'lucide-react';
...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const [isMessagesMenuOpen, setIsMessagesMenuOpen] = useState(false); // New state for messages submenu
  ...
  const navItems = [
    ...
    { to: '/help', icon: <HelpCircle size={20} />, label: 'Help' }, // Added Help
  ];
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2.5 rounded-md transition-all text-sm font-medium ${
                  isActive 
