...
export default function PrintDTRModal({ isOpen, onClose, logs, studentProfile, requiredHours, completedHours }: PrintDTRModalProps) {
    ...
    const [includePageNumbers, setIncludePageNumbers] = useState(true); // New Page Number State
    ...
    const handleGeneratePDF = () => {
        ...
        const drawHeader = (doc: jsPDF, pageNumber: number) => {
            ...
            if (includeLogo && tugmaLogo) {
                try {
                    // Uses the imported logo (20x20 dimensions, placed at x:14, y:10)
                    doc.addImage(tugmaLogo, 'PNG', 14, 10, 20, 20);
                } catch (error) {
            ...
            // Format Specific Month (e.g., "2026-03" -> "March 2026")
            if (timeframe === 'specific_month' && specificMonth) {
                const [year, month] = specificMonth.split('-');
            ...
            // Student Details (Only on Page 1)
            if (pageNumber === 1) {
                doc.setFontSize(11);
        ...
        // --- TABLE ---
        const tableColumn = ["Date", "Task / Activity", "Logged Hrs"];
        ...
        if (signatureMode === 'last_page') {
            ...
            // Add a new page if the signature doesn't fit
        ...
        if (includePageNumbers) {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                // Placed at the very bottom center of every page
